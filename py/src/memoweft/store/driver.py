"""SQLite driver —— 开库 + 建 schema + 版本盖章 + FTS5 探测。移植自 src/store/*(openStores/migrations)。

async 取舍(D-0042):SQLite 本质同步(TS 侧 nodeSqliteDriver 也「保持全链同步」),Python 用 stdlib sqlite3
