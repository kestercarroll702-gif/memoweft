/**
 * MemoWeft 四幕 demo（Phase 4 · §17）——90 秒看懂「认知纪律」为什么值钱。
 *
 ...
 *   1. 记住：「我对花生过敏」→ 固化为高置信度事实 → 可被召回。          说过的话被记住，且带置信度。
 *   2. 纠正：「其实不是我，是我妹妹」→ 旧认知标失效但不删、新认知取代。   纠正有痕，历史可溯。
  ...
  return {
    ...
    async chat(messages: ChatMessage[]): Promise<string> {
      ...
      if (!/JSON/.test(system)) {
        ...
        if (/sister|妹妹|not.*allerg/i.test(last)) return 'The user clarifies it is their sister, not them, who is allergic to peanuts.';
        if (/peanut|花生|allerg/i.test(last)) return 'The user says they are allergic to peanuts.';
        ...
        if (/stress|mood|心情|tired|累/i.test(last)) return 'The user says they have been stressed and in a low mood this week.';
      ...
      const sister = find(/sister|妹妹|not.*allerg/i);
      const peanut = !sister && find(/peanut|花生|allerg/i);
      const americano = find(/americano|美式/i);
      const milkTea = find(/milk tea|bubble|奶茶/i);
      const mood = find(/stress|mood|心情|tired|累/i);

      if (peanut) {
        out.new.push({ content: 'The user is allergic to peanuts', content_type: 'fact', formed_by: 'stated', support_evidence_ids: [peanut.id] });
      }
      if (sister) {
        // 明确纠正：correct 一条 = 指向旧认知 + 新认知内容（consolidate 据此把旧标失效保留、采纳新的）。
        const old = existing.find((c) => /allergic to peanuts/i.test(c.content) && !/sister/i.test(c.content));
        const corrected = { content: "The user's sister is allergic to peanuts", content_type: 'fact', formed_by: 'stated', support_evidence_ids: [sister.id] };
        if (old) out.correct.push({ cognition_id: old.id, ...corrected });
        else out.new.push(corrected);
      }
      if (americano) {
        out.new.push({ content: 'The user likes americano', content_type: 'preference', formed_by: 'stated', support_evidence_ids: [americano.id] });
      }
      if (milkTea) {
        // 行为与旧偏好冲突 → 标 conflict、两条都留，不覆盖。
        const coffee = existing.find((c) => /americano/i.test(c.content));
        if (coffee) out.conflict.push({ cognition_id: coffee.id, support_evidence_ids: [milkTea.id] });
        out.new.push({ content: 'The user ordered milk tea', content_type: 'state', formed_by: 'observed', support_evidence_ids: [milkTea.id] });
      }
      if (mood) {
        out.new.push({ content: 'The user is stressed and in a low mood', content_type: 'state', formed_by: 'stated', support_evidence_ids: [mood.id] });
      }
...
function wordRetriever(): Retriever {
  ...
  return {
    async indexAll(next) { items = [...next]; },
...
function showProfile(core: ReturnType<typeof createMemoWeftCore>) {
  const cogs = core.memory.listCognitions({ subjectId: SUBJECT }).slice().sort((a, b) => a.content.localeCompare(b.content));
  line('   认知状态表 (core.memory.listCognitions):');
  line('   ' + 'cognition'.padEnd(42) + 'type'.padEnd(12) + 'conf'.padEnd(6) + 'status');
  for (const c of cogs) {
    const mark = c.credStatus === 'conflicted' ? ' !! CONFLICT' : c.invalidAt ? ' (invalidated, kept)' : '';
    line('   ' + c.content.padEnd(42) + c.contentType.padEnd(12) + String(c.confidence).padStart(4).padEnd(6) + c.credStatus + mark);
  }
}

async function act1(core: ReturnType<typeof createMemoWeftCore>) {
  line('\n━━ 幕 1 · 记住 ━━  说过的话被记住，且带 MemoWeft 自算的置信度');
  line('User: "I am allergic to peanuts."');
  await core.ingestUserMessage({ content: 'I am allergic to peanuts', subjectId: SUBJECT, occurredAt: BASE });
  await core.updateProfile({ subjectId: SUBJECT });
  line('Memory: ingest → updateProfile(distill→consolidate) → 固化为 fact');
  showProfile(core);
  const hits = await core.recall({ query: 'allergic peanuts food', subjectId: SUBJECT });
  line('   recall("peanut allergy"): ' + hits.map((h) => `${h.content} [${h.credStatus} ${h.confidence}]`).join(' / '));
}

async function act2(core: ReturnType<typeof createMemoWeftCore>) {
  line('\n━━ 幕 2 · 纠正 ━━  纠正有痕，历史可溯（旧认知失效但不删）');
  line('User: "Actually it is not me — my sister is the one allergic to peanuts."');
  await core.ingestUserMessage({ content: 'Actually it is not me, my sister is allergic to peanuts', subjectId: SUBJECT, occurredAt: '2026-01-11T09:00:00.000Z' });
  await core.updateProfile({ subjectId: SUBJECT });
  line('Memory: consolidate.correct → 旧「user allergic」标 invalidAt（保留可溯源）+ 新「sister allergic」');
  showProfile(core);
}
...
async function act3(core: ReturnType<typeof createMemoWeftCore>) {
  ...
  await core.ingestUserMessage({ content: 'I love americano', subjectId: SUBJECT, occurredAt: '2026-01-12T09:00:00.000Z' });
  await core.updateProfile({ subjectId: SUBJECT });
  line('User(behaviour): ordered milk tea again');
  await core.ingestUserMessage({ content: 'ordered milk tea again', subjectId: SUBJECT, occurredAt: '2026-01-13T09:00:00.000Z' });
  await core.updateProfile({ subjectId: SUBJECT });
  line('Memory: consolidate.conflict → 「likes americano」标 conflicted，两条都留，不覆盖');
  showProfile(core);
}

async function act4(core: ReturnType<typeof createMemoWeftCore>, advance: () => void, fastForwardLabel: string) {
  line('\n━━ 幕 4 · 时间 ━━  情绪会过去，事实会留下');
  line('User: "I have been really stressed and in a low mood this week."');
  await core.ingestUserMessage({ content: 'I have been really stressed and in a low mood this week', subjectId: SUBJECT, occurredAt: '2026-01-14T09:00:00.000Z' });
  await core.updateProfile({ subjectId: SUBJECT });
  const q = 'stressed mood low allergic peanuts sister americano';
  ...
  line('   → 上周的「stressed/low mood」淡出；花生（sister）与偏好类事实留存。');
}
...
async function main() {
  ...
  const ffDays = parseDurationDays(argv.includes('--fast-forward') ? argv[argv.indexOf('--fast-forward') + 1] : undefined);
  ...
  const advance = () => { nowMs += ffDays * 24 * 3600 * 1000; };

  const core = createMemoWeftCore({ dbPath: ':memory:', llm: offlineStubLLM(), retriever: wordRetriever(), clock });
  try {
    if (actArg === 0 || actArg === 1) await act1(core);
    if (actArg === 0 || actArg === 2) { if (actArg === 2) await act1(core); await act2(core); }
    if (actArg === 0 || actArg === 3) { if (actArg === 3) { await act1(core); } await act3(core); }
    if (actArg === 0 || actArg === 4) { if (actArg === 4) { await act1(core); } await act4(core, advance, `${ffDays}d`); }
