...
## 下一轮 · 两阶段(建设 → 真用 → 打磨)(2026-07-14 人类拍板 · 边界见 D-0031)

Phase 1–6 收尾后的下一轮。**memoweft 守「库」身份不变**(CLAUDE/AGENTS/Non-goals 不改):产品/服务(REST server · 多租户 · Postgres 后端 · SaaS · Web 管理台 · 多设备同步 = 原 Later 9/10)归 **weftmate**,memoweft 只**留接缝**、不做服务本体。

### 阶段 1(建设)· 顺序 2026-07-18 调整(D-0037):1.2 打头 → 1.3;1.1 移入阶段 2
- **1.2 适配器规模化(打头·D-0037)** —— adapter-kit 已被 6 个适配器证明,增量铺开更多官方适配器。
- **1.3 Python 移植 + 跨语言一致** —— 独立语言线,给"可移植"上跨语言背书。
- ~~1.1 事件类型(`event` ContentType,第 9 型)~~ → **移入阶段 2**(D-0037):dogfood 驱动的特性跟着 dogfood 走,归位而非降级——落地细节不变(填 fact/state 之间"一次性事件"缺口,见 D-0019;走 D-xxxx + api-freeze + 语料 + 提示词 bump + §15.2 重跑)。
- **为 9/10 留接口(接缝审计)** —— 先只读侦察:weftmate 做 REST/多租户/Postgres 后端 · 多设备同步 · 多模态 · Web 管理台,到底要 memoweft 露哪些口(存储后端可换[现钉 node:sqlite/better-sqlite3] / 多模态证据[现仅 text] / 同步[便携包已有]),现有哪些、缺哪些 → **缺的才补(补也是"加接缝"、不做服务本体)**。**官网试用 demo**(简单 WebUI 供体验)= 到时候项,搭在此接缝上:"真体验"需迷你后端(沾服务化、归 weftmate),"演示版"纯前端(memoweft demo/`site/` 可承)。

### 阶段 2(打磨 · dogfood 驱动 · D-0037 扩充)
- 人类在 **weftmate 上实际使用一段时间**(亲自 dogfood 找问题)→ 拿真实使用暴露的问题回头**优化/完善 memoweft**。**1.1 事件类型在此落地**(D-0037);**官网试用 demo(web 页面)也归此阶段**(形态划分仍按 D-0031:真体验归 weftmate、演示版 memoweft 可承)。
- 它是所有"等触发才做"deferred 项的**收割场**:事件类型是否真需要 · Reranker(D-0026)· 命中词/purpose(D-0030)· 相似度阈值调校 · 置信度/衰减手感 · 真实数据量下性能……**真用了才知道哪个触发**。
- 纪律不变(铁律 4):**收割 ≠ 照单全收**——成片真问题 → 走 D-xxxx + 修;一次性 → 记一笔 defer。预演模板 = 2026-07-14 拼豆 dogfood 只读复现(当前 HEAD 已把"一次抬杠→stated 特质"缓解成"inferred/candidate/低置信",残留是缺事件型的低危样本;脚本范式 + weftmate `dogfood/` 目录即现成工具)。
...
- 召回质量 v2:~~相似度阈值~~ ✅ **收口(2026-07-13·测+记文档,不扩 API)**:机制既存(`config.retrieval.minSimilarity`,默认 0=关,`recall.ts:48` 门控)。真实 bge-m3 黄金集实测:两分布重叠、阈值是"修边"——**0.55 甜点**(零召回损失砍 ~9% 噪声),≥0.6 误杀 gold。收益温和 + 值 embedder-specific + 旋钮已在 → 按铁律 4 不扩 API,只把数据背书写进 config 注释 + calibration。~~**召回解释**~~ ✅ **证据链增量已做(2026-07-13·D-0021·人类批准)**:`recall({ explain:true })` → 每条召回认知带 `provenance`(支撑/反证证据链 + summary + 授权位),门面富化、底层不动、纯 additive。经对抗审查加固:provenance 随附 `allowCloudRead/allowInference`(对齐 buildMemoryGraph),宿主转发云模型前可自筛。剩「命中词」半(要贯穿检索器)留后按需。~~**content 过滤**~~ ✅ **已做(2026-07-13·D-0022·人类批准)**:`recall({ contentTypes:[...] })` 按类型允许名单筛 + 每条召回结果暴露 `contentType`;门面过滤、底层只加填字段、纯 additive。对抗审查 0 confirmed。~~**负反馈**~~ ✅ **已做(2026-07-13·D-0023·人类拍板 Mute 语义 + 批准)**:`core.memory.muteCognition({cognitionId, muted, reason})` → 认知加 `mutedAt` 状态位,召回跳过它**但仍 active、仍参与画像演化**(阶梯 mute⊂archive⊂invalidate;铁律 3b 正交不碰 confidence)。schema 加列走 store.migrate 缺列补(非 formal v2,实现验证后取,理由见 D-0023)。对抗审查确认修 1 真回归:muted 曾永久占 top-K 检索槽→饿死召回,加固为**从召回索引排除**(updateProfile filter !mutedAt)。**召回质量 v2 四件全落**(①阈值 ②content过滤 ③解释 ④负反馈)。**尾项两半均 DEFER(2026-07-14·D-0030·人类拍板 defer + 只读侦察 + 对抗验证)**:~~**purpose 过滤**~~ → 无 purpose 字段、`scope` 是 src/ 内死字段(三写入方全不填、无 populator)、contentType 已覆盖已知用例、参考宿主连 contentTypes 都没调过 → 无连贯定义/无 backing data/无消费者,按铁律 4 不做;重启=真实需求 contentType+做实的 scope 都表达不了时。~~③的「命中词」半~~ → 信息在 `RetrievalHit={id,score}` 丢、须贯穿检索器(无门面捷径),且**默认生产 VectorRetriever 路径恒空**(嵌入无离散 term)、只在无-embedder KeywordRetriever 兜底才亮、不能干净 explain-gate、宿主 recall 仍是占位 → 按铁律 4 不做;若将来做,唯一干净形态 `RetrievalHit.terms?: string[]`(additive、触 api-snapshot 一行)已侦定,重启=宿主 explain/记忆气泡 UI 或命中词召回 eval 出现。~~**contentType/新面透传到 MCP/ai-sdk 适配器**~~ ✅ **已端到端做完 6 适配器**(D-0024 两个 + D-0025/27/28/29 其余四个,各带 AD-7;此前"现仅 core.recall 门面"是陈旧行,2026-07-14 校正)
- **索引维护可见性加固(候选·2026-07-18 FTS 缺口调查产物,见 D-0037 附录)**:① `updateProfile` 的 `indexError` 仅落 runLog,离线脚本/宿主看不见——候选在返回处加 console.warn;② weftmate `config-store` 的 ENV_KEYS 只清 `MEMOWEFT_*` 不清 `DLA_*` 旧名——从带 `DLA_EMBED_*` 的终端启动会静默切 vector 模式(桌面启动无此虞);③ 给离线恢复类脚本立规矩:必须打印 `r.indexError`。背景:恢复批 25 条认知因「.env 自动加载选中 VectorRetriever + 嵌入端点未起 + indexError 双重静默」未进 FTS,已确认下轮 updateProfile 自愈、非技术债。
...
- ~~Python 移植与跨语言一致性~~ → **提上下一轮·阶段 1.3**(D-0031,见上「下一轮」节)
- ~~REST server、多租户、pgvector / Postgres 后端~~ → **归 weftmate**(产品/服务层;memoweft 只留接缝·D-0031)
- ~~托管 SaaS、Web 管理界面、多模态证据、CRDT 同步~~ → **归 weftmate**(memoweft 留接口·D-0031)。**官网试用 demo** 例外:简单 WebUI 供体验,到时候搭在接缝上(真体验需迷你后端归 weftmate、演示版纯前端 memoweft 可承)
- ~~大规模新增适配器(本轮至多新增一个作为契约套件试金石)~~ → **提上下一轮·阶段 1.2**(adapter-kit 已被 6 适配器证明·D-0031)
- 把参考宿主做成产品 / 扩成桌面产品路线(仍 Non-goal:产品那摊 = weftmate)
- 拆分开源 / 闭源功能分层
- ~~**加 `event` ContentType**~~ → **提上下一轮·阶段 1.1 打头**(D-0031)。第 9 个类型:确定发生但短暂的一次性事件,如"今天没吃早饭/删了记录";不封顶置信、给中等衰减/过期窗、credStatus 走正常档——填 fact(确定+永久)与 state(临时+封顶)之间的缺口(见 D-0019)。落地走 D-xxxx + api-freeze + 语料 + 提示词 bump + §15.2 重跑。**dogfood 背书**:2026-07-14 拼豆样本(meta 抬杠无处归类被 reify 成低置信 trait)。(来源:2026-07-13 度量清理② D-0019)
