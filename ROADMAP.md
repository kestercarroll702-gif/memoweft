...
- **可注入时钟方案 C 完整性收尾**(2026-07-11 Phase 4 S1-S3,D-0015):`CreateCoreOptions.clock` 已接到三个 store(落库时间)+ 写路径算子(consolidate/attribute)+ 读路径 now(recall 衰减门控),四幕 demo 确定性/fast-forward 成立。**剩 `managementApi`(手动 invalidate/archive 的 invalidAt/archivedAt)、`obs/runLog`(ts)、`asking`(askedAt)的 `new Date()` 未接 clock** —— demo 不碰这些路径故不阻塞;要彻底"全局无散落 new Date()"再补,各算子 deps 加 clock、工厂透传即可(同 S2 手法)。
