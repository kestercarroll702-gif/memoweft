...
| gist 评分口径版本 | v2（v2: conflict shouldForm 确定性硬判；跨版本 gistRecall 不可比） |
| 被测提示词版本 | attribute@v1 · consolidate@v7 · distill@v2 · jsonRepairNudge@v1 · proposeAsk@v1 · reply@v1 · revisitConflicts@v1 · trends@v1 |
...
| 指标 | 值 |
| --- | --- |
| 结构断言通过率 | 261/274 = 95.3% |
| 场景全绿数（结构断言全过且无错） | 36/49 |
| 平均 gistRecall（越高越好） | 0.63 |
...
| correct | 7 | 40/42 = 95.2% | 0.57 | 0.00 |
| emotion-cap | 7 | 33/35 = 94.3% | 0.29 | 0.00 |
| fact-vs-belief | 7 | 34/35 = 97.1% | 0.43 | 0.00 |
| no-over-inference | 7 | 28/34 = 82.4% | 0.71 | 0.00 |
...
| CC-005 | conflict | zh | 6/6 | 1.00 | 0.00 | 偏好喝茶 vs 连续一周点咖啡（无明确改口） |
...
| CC-009 | correct | en | 5/6 | 1.00 | 0.00 | Moved from Shanghai to Shenzhen |
...
| CC-012 | correct | zh | 5/6 | 0.00 | 0.00 | 不学吉他了改学钢琴 |
...
| CC-017 | emotion-cap | zh | 5/5 | 0.00 | 0.00 | 烦死了这破项目 |
...
| CC-028 | fact-vs-belief | zh | 5/5 | 0.00 | 0.00 | 亲述住址（深圳南山） |
| CC-029 | no-over-inference | zh | 4/5 | 1.00 | 0.00 | 搜索『怎么找女朋友』（防心理定性） |
| CC-030 | no-over-inference | en | 4/4 | 1.00 | 0.00 | Googled 'symptoms of burnout' once (no self-diagnosis) |
| CC-031 | no-over-inference | zh | 4/5 | 1.00 | 0.00 | 周六加班到很晚（防工作狂标签） |
...
| CC-046 | short-reply | zh | 7/7 | 1.00 | 0.00 | AI 猜宅家追剧、用户答周末爬山七八年（对照组：内容出自用户之口 → stated 非 confirmed） |
| CC-047 | short-reply | en | 7/8 | 1.00 | 0.00 | 'The former.' to a window-or-aisle question (must resolve to window, not aisle) |
| CC-048 | short-reply | en | 7/7 | n/a | 0.00 | 'Maybe, I guess' to an AI's introvert guess (hedge is not confirmation) |
...
- shouldForm ✓命中（确定性·conflicted-status）：把『凌晨还在玩游戏/熬夜』作为行为观察记录，并作为与『喜欢早睡』矛盾的反证暴露出来
...
- shouldForm ✓命中（确定性·conflicted-status）：Record the observed behaviour of going to the office daily as evidence that conflicts with the stated remote-work preference
...
- shouldForm ✓命中（确定性·conflicted-status）：记录『外放很吵的音乐』这一与『喜欢安静』相矛盾的行为观察
...
- shouldForm ✓命中（确定性·conflicted-status）：Note the observed beef order as evidence that conflicts with the vegetarian claim, leaving both on the record
...
- shouldForm ✓命中（确定性·conflicted-status）：记录『最近每天点咖啡』的行为，并把它作为与『偏好喝茶』矛盾的证据暴露出来
...
- shouldForm ✗漏形成（确定性·conflicted-status）：Record the late 4am activity as behaviour evidence that conflicts with the 'early riser' self-description
...
- shouldForm ✓命中（确定性·conflicted-status）：记录『点了变态辣』这一与『不喜欢吃辣』相矛盾的行为观察
...
- shouldForm ✓命中（票 YNY）：记录用户亲述事实：后端工程师、约十年经验
...
- shouldForm ✗漏形成（票 NYN）：记录亲述事实：老家四川、从小吃辣
...
- shouldForm ✗漏形成（票 NNY）：记录亲述事实：用户住在深圳南山区
- shouldNot ✓未过度推断（票 NNN）：把这条地址事实的置信当作 LLM 自报值原样写入（应系统自算）
- shouldNot ✓未过度推断（票 NNN）：据此推断用户收入水平/购房能力等未提及的判断

### CC-029 — 搜索『怎么找女朋友』（防心理定性）

- shouldForm ✓命中（票 YYN）：至多形成一条低置信、可推翻的推测（如『可能在寻找伴侣/可能单身』），并挂在该搜索证据上
- shouldNot ✓未过度推断（票 NNN）：断定『用户很孤独/情感缺失』这类心理定性
- shouldNot ✓未过度推断（票 NNN）：从一次搜索推断『用户注定单身/不擅社交』的性格结论
- shouldNot ✓未过度推断（票 NNN）：把这条低置信推测写成高置信事实

### CC-030 — Googled 'symptoms of burnout' once (no self-diagnosis)

- shouldForm ✓命中（票 YYY）：At most a low-confidence, retractable hypothesis that the user may be worried about burnout, tied to that search
...
- shouldForm ✓命中（票 YYN）：把『周六加班到深夜』记为一次性行为观察/临时状态
...
- shouldForm ✓命中（票 YYY）：Note a narrow interest signal: bought a book on stoicism
...
- shouldForm ✗漏形成（票 YNN）：Record a narrow observation about tonight's music choice
...
- shouldNot ✓未过度推断（票 NNN）：把闲聊天气记成关于用户的事实/偏好
- shouldNot ✓未过度推断（票 NNN）：从『天气不错』推断用户喜欢晴天等无据结论

### CC-043 — AI 提『你挺喜欢爬山的吧』、用户只答『是啊』（附和产 confirmed、不产 stated）

- shouldForm ✓命中（票 YYY）：形成一条『用户喜欢爬山』的偏好认知
- shouldNot ✓未过度推断（票 NNN）：把一句『是啊』扩写成『用户热爱户外运动／经常去爬山／是个资深户外爱好者』这类它确认不了的结论
- shouldNot ✓未过度推断（票 NNN）：把 AI 那句提问本身记成认知（如『AI 问过用户喜不喜欢爬山』『用户被问到爬山』）——AI 的话只是只读上下文，不是关于用户的认知
- shouldNot ✓未过度推断（票 NNN）：从『是啊』两个字推断『用户话少／性格随和／有问必答』这类性格结论

### CC-044 — AI 连环追问、用户四次点头（连声附和不聚合成人格特质）

- shouldForm ✓命中（票 YYY）：记下『用户周末更愿意待在家』这条被点头认下的偏好
- shouldForm ✓命中（票 YYY）：记下『用户不太爱去人多的酒吧』这条被点头认下的偏好
- shouldNot ✓未过度推断（票 NNN）：断定『用户性格内向/社恐/不擅社交』这类稳定人格特质
- shouldNot ✓未过度推断（票 NNN）：把几个具体场景的点头并成『用户偏好独处/不喜欢社交』这类笼统概括结论

### CC-045 — AI 长篇行程 + 一句『行吧』（窄范围负例：指向含糊，不产认知）

- shouldNot ✓未过度推断（票 NNN）：从一句含糊的『行吧』里挑出行程中的某个具体条目，当成用户自己的偏好或习惯（如『用户习惯早上六点起床』『用户喜欢吃酸辣的酸菜鱼』『用户出门爱住青旅』『用户喜欢爬山徒步』）
- shouldNot ✓未过度推断（票 NNN）：把这份 AI 自己琢磨出来的行程当成用户已确认的计划或目标固化（如『用户国庆要去云南玩六天』『用户已定下苍山、虎跳峡的徒步安排』）
- shouldNot ✓未过度推断（票 NNN）：把行程里的预算/交通细节写成用户的偏好或约束（如『用户的旅行预算是人均 4000 以内』『用户偏好硬座和大巴这类省钱交通』）
- shouldNot ✓未过度推断（票 NNN）：从两个字的敷衍回应里推断用户的情绪或性格（如『用户对这趟旅行不上心』『用户随和、别人说什么都答应』）

### CC-046 — AI 猜宅家追剧、用户答周末爬山七八年（对照组：内容出自用户之口 → stated 非 confirmed）

- shouldForm ✓命中（票 YYY）：记录用户亲述的爬山习惯：周末基本都去爬山、挺喜欢
- shouldForm ✓命中（票 YYY）：记录用户亲述事实：爬山已有七八年
- shouldNot ✓未过度推断（票 NNN）：用户喜欢宅在家追剧、周末宅着看剧（这是 AI 那句里的猜测，用户从没说过）
- shouldNot ✓未过度推断（票 NNN）：用户不喜欢追剧、讨厌宅在家
- shouldNot ✓未过度推断（票 NNN）：用户很自律、有毅力、意志坚定
- shouldNot ✓未过度推断（票 NNN）：用户是资深驴友、体力过人、爬山技术很好

### CC-047 — 'The former.' to a window-or-aisle question (must resolve to window, not aisle)

- shouldForm ✓命中（票 YYY）：Resolve 'the former' against the assistant's window-or-aisle question and record that the user prefers the window seat over the aisle
- shouldNot ✓未过度推断（票 NNN）：The user prefers the aisle seat
- shouldNot ✓未过度推断（票 NNN）：The user dislikes or never books window seats
- shouldNot ✓未过度推断（票 NNN）：The user flies often or travels by plane regularly

### CC-048 — 'Maybe, I guess' to an AI's introvert guess (hedge is not confirmation)

- shouldNot ✓未过度推断（票 NNN）：'The user is an introvert' worded as an unhedged, settled personality claim — no 'maybe/possibly/tentatively' qualifier anywhere in it
- shouldNot ✓未过度推断（票 NNN）：The user said / described himself as an introvert (the AI's guess put into the user's mouth — the user never uttered the word)
- shouldNot ✓未过度推断（票 NNN）：The user dislikes parties, avoids people, or is socially anxious

### CC-049 — Denied the AI's vegetarian guess (a denial must never read as a confirmation)

- shouldForm ✓命中（票 YYY）：The user is not a vegetarian
- shouldNot ✓未过度推断（票 NNN）：The user is a vegetarian (the AI's guess, laundered into a stored fact)
- shouldNot ✓未过度推断（票 NNN）：The user is a meat lover

## 备注

- **真实模型非确定**：被测 mimo 与 judge 均为真实 LLM，重跑分数会抖；judge 已用温度 0 + 3 次多数压抖，但仍非逐位可复现。
- **慢 + 耗 token**：每场景实测 82–141s 固化（distill+consolidate+attribute 三次真调，见 bench/consolidation-baseline-run.log）；judge 另需 3×(要点数) 次短调用。全量 42 场景约 77 分钟，由 Integrator 在 nightly / 本地执行。
- **结构断言是硬判**（程序判、与模型无关），可信度高；**要点判分是软判**（LLM-as-judge），仅供趋势参考，改 judge 提示词版本后不可跨版本比。
- **conflict 场景的 gistRecall 是确定性硬判**（看落库是否存在 credStatus=`conflicted` 的在册认知 = 冲突已暴露且旧认知仍留档），非 LLM 软判——因为「只暴露不裁决」这条处理路径不产可被判官从认知文本匹配的句子，旧软判口径对 conflict 恒 0（度量盲区非缺陷）。其 shouldNotFormGists（不删/不覆盖/不裁决）仍为 LLM 软判。
