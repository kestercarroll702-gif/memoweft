...
- 想跑测试台 / 真实写路径，需要在 `.env` 配模型与嵌入器（见下方「配置」）。**但单元测试不需要任何 .env**——测试用假 LLM，纯离线，67 个全过。

```bash
npm ci            # 或 npm install
npm run typecheck # 类型
npm test          # 单元测试（离线，67 过）
...
| `npm test` | `node --test tests/**/*.test.ts`，纯离线 | `pass 67 / fail 0`（数字随测试增减，`fail` 必须 0） |
