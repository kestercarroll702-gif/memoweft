...
- **eval 口径待议**:Phase 3 后 `created来源⊆{...}`(`eval-consolidation.mjs:312`)测的是**代码**而非模型 → 鉴别力变质;语料的双向设防会退化成「deriveFormedBy 的单测」。是否仍留在 eval 里,收口时决定并在贴分时讲清。
- **可能撞 DoD 钩子**:报告 :241 的「tests/eval/ 既有断言未改(只增新文件)」——若 `cognition-discipline.eval.test.ts` 因 Phase 3 变红需改,**须人类批**。(`bench/` 不在该目录、改它不触钩子。)

**待续**:探针数据 → 定兜底策略 → 实现 `deriveFormedBy` + 单测 → 提示词 v5→v6(教学归宿迁移,非简单删)→ 重写 `confirmedLaundering.test` → §15.3 全量前后对拍 → 治理收口。
