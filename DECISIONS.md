...
**P2-5 已落地(2026-07-18·consolidate 四分支判定 + 落库,Phase 2 最核心·一次对拍通过)**:
- 扩生成器:新增 `parity/consolidate.json`(真 TS consolidate + stub llm 喂固定 LLMOut,zh/en 两组;setup = 3 existing 认知 + 1 事件覆盖 4 spoken 证据 + 短标号 e1-e4;覆盖四分支 + 并存 + resolution)→ dump messages 逐字节 + created 结构 + 计数 + 落库状态。
- Python:`consolidate.py`(四分支 new / reinforce 含**并存 stated** / correct / conflict + `resolution_of` 规整 + `resolve_formed_by` 组 CarrierInput → `derive_formed_by` + `_build_messages` 短标号 `e{n}` + 私有 `_resolve_evidence_id` **只剥 `ev-`** + `pick_support` 用 `dict.fromkeys` 保序 + P2-5 用 noop transaction)。复用已移 confidence/formed_by/echoed_id + jsonRepair + prompts loader + store CRUD + 隐私门。
- 验证:zh/en messages 逐字节(system=consolidate@v7 长提示词 / user=画像+材料短标号)+ created 3 条(new project stated 600 / 并存 preference stated 600 / correct fact stated 600)+ reinforce confidence 320(confirmed 恒继承·支持 2)+ correct 旧失效 + conflict conflicted + resolution 落库(consolidate@v7)+ existing 排序(confidence DESC)+ markConsolidated。gate:**py pytest 58/58 · mypy --strict 0**;**TS shared:check 一致 · npm test 402/402**。
- 诚实分级:四分支判定 + 落库 + 短标号 + 载体维派生 + 并存跨语言坐实(**一次对拍通过**,证移植方法可靠);id 随机不比值(结构对拍)。transaction 用 noop。**待续 P2-6(transaction 可重入 BEGIN/COMMIT/ROLLBACK + 原子回滚)**。

**P2-6 已落地(2026-07-18·transaction 可重入 + 原子回滚,consolidate 完整移植收尾)**:
- Python:`store/transaction.py`(`Transaction` 类型 + `noop_transaction` + `make_transaction(db)` 可重入:最外层真 BEGIN/COMMIT/ROLLBACK、里层直接跑;autocommit 下手动控事务);consolidate 接进(`Transaction`/`noop_transaction` 从 `store.transaction` 引,`run_tx` 缺省 noop)。
- 验证:事务提交/回滚(抛错整段撤销)+ 可重入(里层不嵌套 BEGIN、随外层一起回滚)+ noop + **consolidate 原子性**(markConsolidated 抛错 → new 认知未落、事件仍未消化,不出现「认知写了但事件没标 → 下轮重复」)。gate:**py pytest 62/62 · mypy --strict 0**(纯 py,不触 shared/TS)。
- **consolidate(P2-5 + P2-6)完整移植收尾,Phase 2 最大块搞定 → 待续 P2-7(attribute + trends 背景写路径)**。

**P2-7 已落地(2026-07-18·attribute 归因 + trends 趋势聚合)**:
- 扩生成器:config-constants 的 `attribution` 纳入**全字段**(原只有 `hypothesisCap`;`windowHours`/`maxPhenomenaPerRun`/`maxCausesPerHypothesis`/`minPhenomenonSupport` 是归因规则门,补单一源缺口);新增 `parity/attribute.json` + `parity/trends.json`(真 TS + stub llm,zh/en 各一组)。
- Python:config 补 `Attribution` + `trend_window_days`/`trend_min_count`;clock 补 `ms_to_iso`(整数 timedelta 避 float 误差);`attribute.py`(现象筛选 = 未归因 ∧ 支撑 ≥ `min_phenomenon_support`、按 `updated_at` 降序取 `max_phenomena_per_run`、**禁 state→state**、时间窗 `[anchor-window_hours, 此刻]`、短标号 e1、cited 封顶 `max_causes_per_hypothesis`、支撑 = 原因 + 现象锚、`hypothesis_cap` 封顶);`trends.py`(`all()` 历史口径 + **排除 confirmed** 护栏 + `trend_min_count` 规则门 + dedup + 短标号 + `formed_by=ruled`;**不筛 `allow_inference`**——与 attribute 刻意不同)。
- 验证:zh/en messages 逐字节 + hypotheses(confidence 240 = min(240, cap 250)、credStatus candidate、basedOnCount 2 = 原因+锚)+ trends(confidence 530 = ruled 450 + 2×40、limited、supportCount 3)+ consideredPhenomena/consideredCount/llmCalls。gate:**py pytest 64/64 · mypy --strict 0**;**TS shared:check 一致 · npm test 402/402**。
- **待续 P2-8(asking:proposeAsk/revisitConflicts,先补 `asking.maxAsks` 单一源缺口)**。

**P2-8 已落地(2026-07-18·asking:proposeAsk + revisitConflicts)**:
- 扩生成器:config-constants 的 `asking` 纳入**全字段**(补 `maxAsks` 单一源缺口——候选 slice 依赖它);新增 `parity/asking.json`(proposeAsk/revisitConflicts × 模板路径/LLM 措辞路径 × zh/en = 8 组)。
- Python:config 补 `Asking`/`ConfidenceBand`;`asking.py`(`AskProposal` + `propose_ask` + `revisit_conflicts`:候选筛选(hypothesis ∧ asked_at None ∧ askable_statuses ∧ confidence_band)、按 confidence 降序取 max_asks、**observed 优先稳定分区**、tier 门只筛喂措辞模型的(宿主展示保持完整)、模板 vs LLM 措辞、js_trim + 空回落模板、asked_at 去重写)。
- 验证:zh/en × 4 路径逐字对拍(模板文案 `「」`/`、` 拼接 + LLM messages `【假设】/【证据】`·`【认知】/【支撑证据】/【反对证据】` + evidenceSummaries observed 优先序 + contradictSummaries 两面 + llmCalls + askedAt 写入)。gate:**py pytest 65/65 · mypy --strict 0**;**TS shared:check 一致 · npm test 402/402**。
- **待续 P2-9(updateProfile 编排:distill→consolidate→attribute 串链 + 重索引)**。

**P2-9 已落地(2026-07-18·updateProfile 编排,Phase 2 移植部分收口)**:
- 扩生成器:新增 `parity/update-profile.json`(真 TS updateProfile + 按序 stub llm(① distill 摘要 ② consolidate JSON)+ stub retriever;zh/en)。
- Python:`update_profile.py`(`Retriever` Protocol + distill→consolidate→attribute→重索引串链 + `timings`/`metrics` + **索引失败不回滚画像**(index_error 记下);索引只含 active ∧ 未 muted)。
- 验证:zh/en 全链对拍(distill eventSummary/pending/llmCalls → consolidate created/processedEvents/profileSize/**promptChars 3811(UTF-16)** → attribute 无 state 现象不调模型 → indexed/indexedTexts/metrics/totalLlmCalls 2)+ 索引失败不回滚专项(检索器抛错 → index_error 记下、画像照落库)。gate:**py pytest 67/67 · mypy --strict 0**;**TS shared:check 一致 · npm test 402/402**。
- **Phase 2【移植】部分全部完成**(底座 + LLM 层 + 全部写路径 + 表达 + 编排)→ 待续 P2-10(§15.3 eval harness,属**验证**)与 P2-旁(便携包硬化)。
