 ...
 *   FormedBy      = stated|observed|ruled|inferred
...
const FORMED_BY = ['stated', 'observed', 'ruled', 'inferred'];
const CRED_STATUS = ['candidate', 'low', 'limited', 'stable', 'conflicted'];
const SOURCE_KIND = ['spoken', 'inferred', 'observed'];
const DISCIPLINES = ['conflict', 'correct', 'emotion-cap', 'fact-vs-belief', 'no-over-inference', 'chitchat-negative'];
const LANGS = ['zh', 'en', 'mixed'];

interface Expect {
  conflict: boolean;
  correct: boolean;
  newCognitions: { min: number; max: number; types?: string[] };
...
test('CORP-13 非 conflict/correct 纪律：expect.conflict===false 且 correct===false', () => {
  const others = ['emotion-cap', 'fact-vs-belief', 'no-over-inference', 'chitchat-negative'];
