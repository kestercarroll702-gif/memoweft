...
test('confirmed（附和，D-0033）：底分弱 + 自然封顶 → 纯附和永不达 limited/stable（结构对抗护栏·把不变式钉死）', () => {
  // 附和排序：inferred(200) < confirmed(280) < observed(350) < ruled(450) < stated(600)。
  const inferred = computeConfidence({ contentType: 'fact', formedBy: 'inferred', supportCount: 1, contradictCount: 0 });
  const confirmed = computeConfidence({ contentType: 'fact', formedBy: 'confirmed', supportCount: 1, contradictCount: 0 });
  const observed = computeConfidence({ contentType: 'fact', formedBy: 'observed', supportCount: 1, contradictCount: 0 });
  const stated = computeConfidence({ contentType: 'fact', formedBy: 'stated', supportCount: 1, contradictCount: 0 });
  assert.ok(inferred < confirmed && confirmed < observed && observed < stated, `来源强度排序：${inferred}<${confirmed}<${observed}<${stated}`);
  // 单次附和 = 280 = 候选（<low 300）。
  ...
  const confirmedMax = computeConfidence({ contentType: 'fact', formedBy: 'confirmed', supportCount: 50, contradictCount: 0 });
  assert.ok(confirmedMax <= 480, `纯附和封顶 ≤480（实际 ${confirmedMax}）`);
  assert.ok(confirmedMax < 500, '纯附和永远够不到 limited(500) —— 只有用户主动说（升级 formedBy）才破顶');
  assert.equal(deriveCredStatus(confirmedMax, 0, 'fact'), 'low', '纯附和攒满也顶天低置信，永不 limited/stable');
});
...
test('updateProfile：M4 归因自动并进（更新画像顺带对新现象产假设）', async () => {
  ...
  try {
    ...
    const eGame = ev.put({ subjectId: 'owner', sourceKind: 'observed', hostId: 'h', rawContent: '游戏开到凌晨3:30', occurredAt: '2026-06-23T03:30:00.000Z', allowCloudRead: true }); // observed 默认不上云已下沉 put；attribute 当云可读候选原因用，显式授权（路线 A）
