/**
 * KeywordRetriever 测试（Phase 1 · §14.3：FTS5 trigram + BM25 关键词召回）。
 * 全部用 ':memory:' 库、零网络、零嵌入。先红后绿。
 *
 * 覆盖：term 命中相关 doc / BM25 稀有词更靠前 / 3 字中文命中且 2 字中文返回 []（D-0001）/
...
import {
  KeywordRetriever,
  FtsUnavailableError,
} from '../../src/retrieval/keywordRetriever.ts';
...
test('BM25：更稀有的词把其 doc 排更前', async () => {
  ...
  try {
    ...
    const commonBest = Math.max(
      ...hits.filter((h) => h.id !== 'd1').map((h) => h.score),
    );
...
test('CJK：3 字中文 query 命中；2 字中文 query 返回 [](D-0001：trigram <3 字符无输出)', async () => {
  const r = new KeywordRetriever(':memory:');
  try {
    await r.indexAll([
    ...
    assert.deepEqual(hit2, [], '2 字中文 query 返回 []（trigram 限制，D-0001）');
  } finally {
    r.close();
  }
});
...
test('增量：indexAll 改内容后 search 反映更新；移除的 id 不再召回', async () => {
  ...
  try {
    ...
    assert.ok(hikeHits.some((h) => h.id === 'a'), '新词 hiking 命中更新后的 a');
    // 被移除的 b 不再召回。
...
test('query 含 FTS5 特殊字符（" * ( ) : ^）不抛错', async () => {
  ...
  try {
    ...
    assert.ok(hits.some((h) => h.id === 'a'), '消毒后 peanut 仍命中 a');
  } finally {
    r.close();
  }
});
...
test('unicode61 tokenizer 可配（纯英文场景），基本召回可用', async () => {
  ...
  try {
    ...
    assert.ok(hits.some((h) => h.id === 'a'), 'unicode61 下 fox 命中 a');
  } finally {
    r.close();
  }
});

test('FtsUnavailableError 是具名错误（供 §14.3b 工厂 catch 后降级）', () => {
