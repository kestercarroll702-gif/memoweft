...
test('维度恒为 DIM；非空文本模长≈1，空/纯空白文本全零、模长 0', async () => {
  ...
  assert.ok(empty.every((x) => x === 0), '空文本全零');
  assert.equal(norm(empty), 0, '空文本模长 0');
  assert.ok(blank.every((x) => x === 0), '纯空白文本无 token → 全零');
...
test('2 字中文词与含它的文本有 token 重叠（D-0001：char-bigram+单字）', async () => {
  const e = new HashEmbedder();
