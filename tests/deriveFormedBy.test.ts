/**
 * deriveFormedBy —— 载体维派生 + 取最弱聚合 + 兜底（v0.6 Phase 3 · D-0035）。
 *
 * **为什么这个文件承重**：D-0035 拍板② 的「多证据取最弱」在 §15.3 全量 eval 上**零鉴别力**——
 * 侦察查证 eval 语料的**异质支持集 0/49**（47 条只有 1 条 message → 一条认知最多引 1 条证据是物理上限），
 * 取最强/取最弱/分裂/主证据四种策略在那 49 场景上产出**完全相同**。所以洗白防线只能靠本文件钉住。
 *
 * 规范 = `docs/internal/v0.6-impact-report.md:84-96` 的派生表；设计与取舍见 `src/consolidation/deriveFormedBy.ts` 文件头。
 */
...
const res = (propositionOrigin: PropositionOrigin | null, responseAct: ResponseAct | null = null) => ({ propositionOrigin, responseAct });
...
test('表第 4 行：spoken ∧ assistant_proposed ∧ affirm（附和）→ confirmed，绝不 stated', () => {
  assert.equal(
    deriveFormedBy([ev({ precedingAiContext: '你挺喜欢爬山的吧?', resolution: res('assistant_proposed', 'affirm') })]),
    'confirmed',
  );
});

test('表第 6 行：spoken ∧ assistant_proposed ∧ select（二选一里选）→ confirmed（2026-07-16 人类补拍）', () => {
  assert.equal(
    deriveFormedBy([ev({ precedingAiContext: 'window or aisle?', resolution: res('assistant_proposed', 'select') })]),
    'confirmed',
  );
});

test('表 :90：assistant_proposed ∧ negate（用户否认 AI 的猜测）→ stated——被断言的否定命题是用户自己的明确表达', () => {
  assert.equal(
    deriveFormedBy([ev({ precedingAiContext: "You're a vegetarian, right?", resolution: res('assistant_proposed', 'negate') })]),
    'stated',
  );
});
...
test('表未覆盖的组合：assistant_proposed ∧ elaborate/ask/other → 收敛到 confirmed（命题是 AI 提的⇒载体不是用户）', () => {
  for (const act of ['elaborate', 'ask', 'none', 'other', null] as (ResponseAct | null)[]) {
    assert.equal(
      deriveFormedBy([ev({ precedingAiContext: '你喜欢爬山吧?', resolution: res('assistant_proposed', act) })]),
      'confirmed',
...
test('兜底·解析里 propositionOrigin 收敛成 null（非法枚举）→ 同无解析处理', () => {
  assert.equal(deriveFormedBy([ev({ precedingAiContext: null, resolution: res(null, 'affirm') })]), 'stated');
  assert.equal(deriveFormedBy([ev({ precedingAiContext: '你喜欢爬山吧?', resolution: res(null, 'affirm') })]), 'confirmed');
});
...
test('取最弱：强弱序 confirmed(280) < observed(350) < stated(600)，锚定 config.baseByFormedBy 底分', () => {
  ...
  const confirmed = ev({ precedingAiContext: '你喜欢爬山吧?', resolution: res('assistant_proposed', 'affirm') });
  ...
  assert.equal(deriveFormedBy([observed, confirmed]), 'confirmed', '[observed, confirmed] → confirmed');
  assert.equal(deriveFormedBy([stated, observed, confirmed]), 'confirmed', '三者齐 → 最弱的 confirmed');
  assert.equal(deriveFormedBy([stated, stated]), 'stated', '同质不降级');
});

test('取最弱与顺序无关', () => {
  const stated = ev({ resolution: res('user_stated', 'elaborate') });
  const confirmed = ev({ precedingAiContext: 'x?', resolution: res('assistant_proposed', 'affirm') });
  assert.equal(deriveFormedBy([stated, confirmed]), deriveFormedBy([confirmed, stated]));
});

// ── 洗白防线（本文件的核心；D-0035 拍板② 否掉「取最强」的正是这条）──────────

test('反洗白：附和 + 顺带引一条无关的主动陈述 → 仍是 confirmed，绝不 stated', () => {
  // 攻击场景（侦察坐实、不需要恶意模型）：pickSupport 只查 id 白名单、不查相关性，validEvidence 覆盖整批
  ...
  const affirmed = ev({ precedingAiContext: '你是不是特别不爱打电话?', resolution: res('assistant_proposed', 'affirm') });
...
test('反洗白：附和 + 五条无关主动陈述（取最强会到 800/stable）→ 仍是 confirmed', () => {
  const affirmed = ev({ precedingAiContext: '你不太爱去人多的酒吧?', resolution: res('assistant_proposed', 'affirm') });
...
test('不返回 inferred / ruled——载体维只有三个取值（D-0035 拍板①：推断距离仍由模型报）', () => {
