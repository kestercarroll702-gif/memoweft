...
function attrScenario() {
  ...
  const eGame = ev.put({ subjectId: 'owner', sourceKind: 'observed', hostId: 'h', rawContent: '游戏开到凌晨 3:30', occurredAt: '2026-06-23T03:30:00.000Z', allowCloudRead: true });
  cog.put({
    subjectId: 'owner', content: '用户昨晚没睡好', contentType: 'state', formedBy: 'stated', confidence: 250, credStatus: 'low',
    evidence: [{ evidenceId: eSleep.id, relation: 'support' }, { evidenceId: eSleep2.id, relation: 'support' }],
  });
  return { ev, cog, eGame };
}

test('attribute · 标号路（治本正路）：模型写 e1 → 假设产出、溯源挂真证据 id', async () => {
  const { ev, cog, eGame } = attrScenario();
  try {
    const r = await attribute('owner', { evidenceStore: ev, cognitionStore: cog, llm: stubOf('{"hypotheses":[{"content":"可能因为玩游戏太晚导致没睡好","based_on_evidence_ids":["e1"]}]}') });
...
test('attribute · 截断兜底：模型把 UUID 截成前 8 位 → 仍解回、假设不再静默丢弃', async () => {
  const { ev, cog, eGame } = attrScenario();
  try {
    const r = await attribute('owner', { evidenceStore: ev, cognitionStore: cog, llm: stubOf(`{"hypotheses":[{"content":"可能因为玩游戏太晚导致没睡好","based_on_evidence_ids":["${eGame.id.slice(0, 8)}"]}]}`) });
...
test('attribute · 护栏：捏造 id（非任何真 id 前缀）→ 不产假设（3a 不松动）', async () => {
  const { ev, cog } = attrScenario();
  try {
    const r = await attribute('owner', { evidenceStore: ev, cognitionStore: cog, llm: stubOf('{"hypotheses":[{"content":"瞎编","based_on_evidence_ids":["fabricated-cause-id"]}]}') });
...
test('trends · 标号路：模型写 e1..e4 → 趋势产出、溯源挂真证据 id', async () => {
  ...
  try {
    ...
    const r = await aggregateTrends('u', { evidenceStore: ev, cognitionStore: cog, llm: stubOf('{"trends":[{"content":"用户最近这段时间持续情绪低落","based_on_evidence_ids":["e1","e2","e3","e4"]}]}') }, new Date('2026-06-30T00:00:00.000Z'));
    assert.equal(r.trends.length, 1, '标号应解回 → 聚出趋势');
    const linked = new Set(cog.sourcesOf(r.trends[0]!.id).map((l) => l.evidenceId));
    assert.ok(ids.every((id) => linked.has(id)), '溯源挂 4 条真证据 id');
  } finally { ev.close(); cog.close(); }
});
...
test('trends · 截断兜底：模型把 UUID 截成前 8 位 → 趋势不再静默丢弃', async () => {
  ...
  try {
    ...
    const r = await aggregateTrends('u', { evidenceStore: ev, cognitionStore: cog, llm: stubOf(`{"trends":[{"content":"用户最近这段时间持续情绪低落","based_on_evidence_ids":${trunc}}]}`) }, new Date('2026-06-30T00:00:00.000Z'));
...
test('trends · 护栏：捏造 id → 不聚趋势', async () => {
  ...
  try {
    ...
    const r = await aggregateTrends('u', { evidenceStore: ev, cognitionStore: cog, llm: stubOf('{"trends":[{"content":"瞎编","based_on_evidence_ids":["fake-a","fake-b","fake-c"]}]}') }, new Date('2026-06-30T00:00:00.000Z'));
...
interface CStores { ev: SqliteEvidenceStore; evt: SqliteEventStore; cog: SqliteCognitionStore; sr: SqliteSemanticResolutionStore; }
function cFresh(): CStores {
  return { ev: new SqliteEvidenceStore(':memory:'), evt: new SqliteEventStore(':memory:'), cog: new SqliteCognitionStore(':memory:'), sr: new SqliteSemanticResolutionStore(':memory:') };
}
function cClose(s: CStores) { s.ev.close(); s.evt.close(); s.cog.close(); s.sr.close(); }
...
function reinforceSetup(s: CStores) {
  const cogId = s.cog.put({ subjectId: 'u', content: '用户喜欢喝茶', contentType: 'preference', formedBy: 'stated', confidence: 600, credStatus: 'limited' }).id;
  const e = s.ev.put({ subjectId: 'u', sourceKind: 'spoken', hostId: 'h', rawContent: '我还是很爱喝茶', occurredAt: '2026-06-01T08:00:00.000Z' });
  s.evt.put({ subjectId: 'u', summary: '用户重申爱喝茶', occurredAt: '2026-06-01T08:00:00.000Z', evidenceIds: [e.id] });
...
test('cognition_id · 截断容错：模型把 cognition_id 截成前 8 位 → reinforce 仍生效', async () => {
  ...
  try {
    ...
    const body = JSON.stringify({ reinforce: [{ cognition_id: cogId.slice(0, 8), support_evidence_ids: ['e1'] }] });
    const r = await consolidate('u', cDeps(s, stubOf(body)));
    assert.equal(r.reinforced, 1, '截断的 cognition_id 应被解回 → reinforce 生效（此前：get 落空 → 静默跳过）');
    assert.ok(s.cog.sourcesOf(cogId).some((l) => l.evidenceId === eId), '新证据挂到了真认知上');
  } finally { cClose(s); }
});
...
test('cognition_id · 护栏 + 告警：认不出的 cognition_id → 不 reinforce 且落告警', async () => {
  ...
  try {
    reinforceSetup(s);
    const body = JSON.stringify({ reinforce: [{ cognition_id: 'totally-fake-cog-id', support_evidence_ids: ['e1'] }] });
    const r = await consolidate('u', cDeps(s, stubOf(body)));
...
test('cognition_id · 写对完整 id 时零变化（精确匹配优先）', async () => {
  ...
  try {
    ...
    const r = await consolidate('u', cDeps(s, stubOf(JSON.stringify({ reinforce: [{ cognition_id: cogId, support_evidence_ids: ['e1'] }] }))));
