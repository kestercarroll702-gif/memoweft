...
test('OpenAICompatClient.chat：接 usage（snake_case）、多次累加、callsWithUsage 计数', async () => {
  ...
  try {
    const c = chatClientWith([
      { choices: [{ message: { content: 'a' } }], usage: { prompt_tokens: 10, completion_tokens: 5, total_tokens: 15 } },
      { choices: [{ message: { content: 'b' } }], usage: { prompt_tokens: 20, completion_tokens: 8, total_tokens: 28 } },
    ]);
    await c.chat([{ role: 'user', content: 'x' }]);
    await c.chat([{ role: 'user', content: 'y' }]);
    assert.deepEqual(c.usage, { promptTokens: 30, completionTokens: 13, totalTokens: 43, callsWithUsage: 2 });
...
test('OpenAICompatClient.chat：端点不回 usage → 计数保持 0、不崩（本地模型常见）', async () => {
  ...
  try {
    ...
    assert.deepEqual(c.usage, { promptTokens: 0, completionTokens: 0, totalTokens: 0, callsWithUsage: 0 });
...
test('OpenAICompatClient.chat：total_tokens 缺失 → 回退 prompt+completion', async () => {
  const orig = globalThis.fetch;
  try {
    const c = chatClientWith([{ choices: [{ message: { content: 'a' } }], usage: { prompt_tokens: 7, completion_tokens: 3 } }]);
    await c.chat([{ role: 'user', content: 'x' }]);
...
test('OpenAICompatEmbedder.embed：接 usage（无 completion）、空输入不计数', async () => {
  ...
  try {
    const stub = async (): Promise<Response> =>
      ({
        ok: true,
        json: async () => ({ data: [{ embedding: [0.1, 0.2] }], usage: { prompt_tokens: 12, total_tokens: 12 } }),
    ...
    assert.deepEqual(e.usage, { promptTokens: 12, completionTokens: 0, totalTokens: 12, callsWithUsage: 1 }, '嵌入无 completion');
  } finally {
    globalThis.fetch = orig;
  }
});
...
test('core.usage()：chat/write 同实例去重不重复计；llm/embed 分桶 + 合计', () => {
  ...
  try {
    const u = core.usage();
    assert.deepEqual(u.llm, { promptTokens: 100, completionTokens: 40, totalTokens: 140, callsWithUsage: 3 }, 'chat/write 同实例去重');
    assert.deepEqual(u.embed, { promptTokens: 50, completionTokens: 0, totalTokens: 50, callsWithUsage: 2 });
    assert.deepEqual(u.total, { promptTokens: 150, completionTokens: 40, totalTokens: 190, callsWithUsage: 5 }, 'llm + embed 合计');
  } finally {
    core.close();
  }
});
