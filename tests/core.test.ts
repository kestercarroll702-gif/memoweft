/**
 * createMemoWeftCore 统一入口（架构归位·批次2）。
 * 用 stub LLM / 伪 retriever，不依赖网络与嵌入器；库用 :memory: 或临时文件（用完即清）。
 *
 * 验的契约：
 *  - 工厂缺 .env 不崩（LLM 缺→真调用才报；嵌入缺→KeywordRetriever 关键词兜底,D-0017）。
...
test('ingestObservation：observed 证据落库，默认不上云（observedDefaults）', async () => {
  ...
  try {
    const stored = await core.ingestObservation({
      observations: [{ kind: 'active_window', occurredAt: new Date().toISOString(), content: '在 VS Code 停留约 40 分钟' }],
    });
...
test('core.recall：走共享召回门控（invalid / archived 不出，正常召回带 id）', async () => {
  ...
  try {
    ...
    const keep = seed.cognitionStore.put({ subjectId: 'owner', content: '用户喜欢喝茶', contentType: 'preference', formedBy: 'stated', confidence: 900, credStatus: 'stable' });
    const dead = seed.cognitionStore.put({ subjectId: 'owner', content: '用户喜欢咖啡（已失效）', contentType: 'preference', formedBy: 'stated', confidence: 900, credStatus: 'stable' });
    const filed = seed.cognitionStore.put({ subjectId: 'owner', content: '用户在学吉他（已归档）', contentType: 'preference', formedBy: 'stated', confidence: 900, credStatus: 'stable' });
...
test('D-0017 无 embedder 兜底 KeywordRetriever：不注入 embedder/retriever → keyword 召回非空（且 embedReady=false）', async () => {
  // 契约（D-0017,§14.4b 重评估）:没配 embedder 时,recall 不再恒空——走 FTS5 关键词兜底。
  ...
  const EMBED_KEYS = ['MEMOWEFT_EMBED_BASE_URL', 'MEMOWEFT_EMBED_API_KEY', 'MEMOWEFT_EMBED_MODEL', 'DLA_EMBED_BASE_URL', 'DLA_EMBED_API_KEY', 'DLA_EMBED_MODEL'];
  const saved = Object.fromEntries(EMBED_KEYS.map((k) => [k, process.env[k]]));
  for (const k of EMBED_KEYS) process.env[k] = '';
  try {
    const seed = openStores(dbPath);
    seed.cognitionStore.put({ subjectId: config.identity.subjectId, content: '用户喜欢徒步旅行', contentType: 'preference', formedBy: 'stated', confidence: 900, credStatus: 'stable' });
    seed.close();
    ...
    try {
      ...
      assert.ok(out.length >= 1, 'D-0017:无 embedder 也能经 keyword 召回（非恒空）');
      assert.equal(out[0]!.content, '用户喜欢徒步旅行', 'keyword 命中正确认知');
    } finally {
      core.close(); // KeywordRetriever 连接被收口（不抛错）
    }
  } finally {
    for (const k of EMBED_KEYS) { if (saved[k] === undefined) delete process.env[k]; else process.env[k] = saved[k]; }
...
test('handleConversationTurn：同 conversationId 复用实例，窗口跨轮连续', async () => {
  ...
  try {
    const first = await core.handleConversationTurn({ message: '第一轮的话', systemPrompt: '你是测试人设。' });
    ...
    assert.ok(secondMessages.some((m) => m.role === 'system' && m.content.includes('你是测试人设')), '首建时的 systemPrompt 持续生效');
  } finally {
    core.close();
  }
});
...
test('dropConversation：丢弃实例后同 conversationId 重建，换 systemPrompt 才生效', async () => {
  ...
  const core = createMemoWeftCore({ dbPath: ':memory:', llm, retriever: nullRetriever });
  try {
    await core.handleConversationTurn({ message: '一', conversationId: 'x', systemPrompt: '人设A。' });
    assert.ok(llm.calls[0]!.some((m) => m.role === 'system' && m.content.includes('人设A')), '首建用人设A');

    // 不 drop：复用旧实例，新 systemPrompt B 被【忽略】——这正是插件切换审查抓出的坑（也是行为冒烟的假阳性来源）。
    await core.handleConversationTurn({ message: '二', conversationId: 'x', systemPrompt: '人设B。' });
    assert.ok(llm.calls[1]!.some((m) => m.role === 'system' && m.content.includes('人设A')), '不 drop → 复用旧实例、仍人设A');
    assert.ok(!llm.calls[1]!.some((m) => m.role === 'system' && m.content.includes('人设B')), '不 drop → 新人设B 被忽略');

    // drop 后：同 id 重建实例，新 systemPrompt C 生效、窗口重置（没传 seedTurns → 不带旧轮）。
    core.dropConversation('x');
    await core.handleConversationTurn({ message: '三', conversationId: 'x', systemPrompt: '人设C。' });
    assert.ok(llm.calls[2]!.some((m) => m.role === 'system' && m.content.includes('人设C')), 'drop 后 → 重建实例、人设C 生效');
...
test('portable / graph 薄封装：subjectId 缺省单例，导出→校验→dryRun 导入闭环', async () => {
  const core = createMemoWeftCore({ dbPath: ':memory:', llm: stubLLM(), retriever: nullRetriever });
  const other = createMemoWeftCore({ dbPath: ':memory:', llm: stubLLM(), retriever: nullRetriever });
  try {
    ...
    assert.ok(payload.nodes.some((n) => n.kind === 'subject'), '图谱含 subject 中心节点');
  } finally {
    core.close();
    other.close();
  }
  ...
  try {
    ...
    const bundle = {
      ...
      data: {
        ...
        eventEvidence: [],
        cognitions: [{
          id: 'cog-1', subjectId: config.identity.subjectId, content: '要归档的认知', contentType: 'fact' as const,
          formedBy: 'stated' as const, confidence: 700, credStatus: 'limited' as const, scope: null,
          validAt: null, invalidAt: null, askedAt: null, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
        }],
        cognitionEvidence: [],
    ...
    assert.ok(shown.nodes.some((n) => n.id === 'cog-1' && n.colorKey === 'archived'), 'includeArchived 才可见');
  } finally {
    core.close();
  }
});
...
test('health：注入真 VectorRetriever → embedReady=true（反映实际召回能力）', () => {
  ...
  const embedder = { async embed() { return []; } };
