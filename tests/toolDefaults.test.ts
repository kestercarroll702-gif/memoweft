/**
 * AD-3 隐私雷的离线护栏（D-0013，地图 cell 8 隐私规则）：
...
test('put tool 无显式授权 → 套 toolDefaults（local✓ / cloud✗ / infer✓）', () => {
  ...
  try {
    ...
    const e = ev.put({ subjectId: 'owner', sourceKind: 'tool', hostId: 'h', rawContent: '{"city":"Xiamen","temp":31}' });
    assert.equal(e.allowCloudRead, false, 'tool 默认不上云（AD-3 隐私雷已拆）');
...
test('put tool 显式 allowCloudRead:true → 尊重显式 true（不被兜底覆盖）', () => {
  const ev = new SqliteEvidenceStore(':memory:');
  try {
    const e = ev.put({ subjectId: 'owner', sourceKind: 'tool', hostId: 'h', rawContent: '工具结果', allowCloudRead: true });
...
test('回归：observed 兜底与 spoken 通用默认不受 tool 分支影响', () => {
  const ev = new SqliteEvidenceStore(':memory:');
  try {
    const observed = ev.put({ subjectId: 'owner', sourceKind: 'observed', hostId: 'h', rawContent: '窗口观察' });
    assert.equal(observed.allowCloudRead, false, 'observed 仍走 observedDefaults 不上云');
    const spoken = ev.put({ subjectId: 'owner', sourceKind: 'spoken', hostId: 'h', rawContent: '亲口说的' });
