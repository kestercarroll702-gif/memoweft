/**
 * 受控记忆管理 API（架构归位·批次2，路线 §5.3）。
 *
 * 记忆管理不是普通 CRUD：每个操作负责引用完整性 + 原因留痕（审计表 management_log）。
 * Host 的记忆管理页应调这里，不再直接摸 Sqlite*Store（boundaries.md §4.3 待纠正项的替身）。
 *
...
/** 受控记忆管理 API（core.memory）。7 个操作 + 独立审计表（用户拍板）。 */
export interface MemoryManagementAPI {
  ...
  // ── 只读列取（批次5 步0 缺口A）：只读不写、不落审计——展示用，不是管理变更 ──
   ...
   *  审计不为本次操作留行——整表都清了，留了也被 clear() 冲掉（批次3 用户拍板：出厂=无历史）。 */
  resetSubject(input: ResetSubjectInput): ResetSubjectResult;
}

/** 装配受控管理 API 的可选依赖（批次5 步0）。 */
 ...
 * @param cfg 可注入配置（P2-5 口径）：不传 = 全局单例；merge 重算置信度、列取/出厂缺省 subject 按这份。
 * @param deps 可选依赖（批次5 步0）：retriever 供 resetSubject 清向量索引；不传则出厂跳过清索引。
 */
export function createMemoryManagementAPI(
  ...
  function blockersOf(evidenceId: string): RemovalBlocker[] {
    ...
    for (const r of cogRows) out.push({ kind: 'cognition', id: r.cognition_id, relation: r.relation });
    return out;
  }
  ...
  return {
    ...
    updateEvidenceAuthorization({ evidenceId, allowCloudRead, allowInference, reason }) {
      return transaction(() => {
        ...
        if (nextCloud === before.allowCloudRead && nextInfer === before.allowInference) return before;
        ...
        managementLog.append({
          ...
          detail: {
            before: { allowCloudRead: before.allowCloudRead, allowInference: before.allowInference },
            after: { allowCloudRead: updated!.allowCloudRead, allowInference: updated!.allowInference },
          },
        });
        return updated;
      });
    ...
    removeCognitionSafely({ cognitionId, reason }) {
      return transaction(() => {
        ...
        managementLog.append({
          ...
          reason,
          // 不存内容原文（批次3 用户拍板）：删除审计只留元数据——被删的是什么类型/哪来的/什么状态/断了几条链。
          detail: { contentType: cur.contentType, formedBy: cur.formedBy, credStatus: cur.credStatus, linkCount: removedLinks.length },
        });
    ...
    mergeCognition({ sourceId, targetId, reason }) {
      ...
      return transaction(() => {
        ...
        for (const l of sourceLinks) {
          ...
          if (targetKeys.has(key)) { duplicateLinks++; continue; } // target 已有同款 → 去重丢弃
        ...
        //    假设类加 hypothesisCap 封顶（同 attribute 产假设的纪律：合并不能把"低声说的假设"抬成定论）。
        ...
        let confidence = computeConfidence(
          { contentType: target.contentType, formedBy: target.formedBy, supportCount, contradictCount },
          cfg,
        );
        if (target.contentType === 'hypothesis') confidence = Math.min(confidence, cfg.attribution.hypothesisCap);
        ...
        return { merged: true, movedLinks: toMove.length, duplicateLinks, target: updatedTarget, source: updatedSource };
      });
    },
    ...
    checkIntegrity() {
      ...
      const eeMissingEvent = db
        .prepare('SELECT ee.event_id, ee.evidence_id FROM event_evidence ee LEFT JOIN event e ON e.id = ee.event_id WHERE e.id IS NULL')
        .all() as unknown as Array<{ event_id: string; evidence_id: string }>;
      for (const r of eeMissingEvent) {
        issues.push({ kind: 'orphan_event_evidence', eventId: r.event_id, evidenceId: r.evidence_id, missing: 'event' });
      }
      const eeMissingEvidence = db
        .prepare('SELECT ee.event_id, ee.evidence_id FROM event_evidence ee LEFT JOIN evidence ev ON ev.id = ee.evidence_id WHERE ev.id IS NULL')
        .all() as unknown as Array<{ event_id: string; evidence_id: string }>;
      for (const r of eeMissingEvidence) {
        issues.push({ kind: 'orphan_event_evidence', eventId: r.event_id, evidenceId: r.evidence_id, missing: 'evidence' });
      }
      // 孤儿 cognition_evidence：cognition 端或 evidence 端指向不存在的行。
      const ceMissingCog = db
        .prepare('SELECT ce.cognition_id, ce.evidence_id FROM cognition_evidence ce LEFT JOIN cognition c ON c.id = ce.cognition_id WHERE c.id IS NULL')
        .all() as unknown as Array<{ cognition_id: string; evidence_id: string }>;
      for (const r of ceMissingCog) {
        issues.push({ kind: 'orphan_cognition_evidence', cognitionId: r.cognition_id, evidenceId: r.evidence_id, missing: 'cognition' });
      }
      const ceMissingEvidence = db
        .prepare('SELECT ce.cognition_id, ce.evidence_id FROM cognition_evidence ce LEFT JOIN evidence ev ON ev.id = ce.evidence_id WHERE ev.id IS NULL')
        .all() as unknown as Array<{ cognition_id: string; evidence_id: string }>;
