/**
 * MemoWeft 包入口：MemoWeft 是被宿主 import 的框架/库。
 * ：证据层 + 存储/召回接口 + 对话源 + 回话编排 + 可观测日志。
 */
...
// 感知 / 多源摄入：Core 只保留【通用观察摄入口】（generic Observation + ingestObservations）。
// [stable] Observation：跨层契约「采集插件→Host→Core」，ingestObservation 入参（meta 字段/kind 开放集见契约 experimental）。
export { type Observation } from './perception/ingest.ts';
// [internal] ingestObservations 算子 + 其 Deps/Result：门面 ingestObservation 内部用。
export { ingestObservations, type IngestDeps, type IngestResult } from './perception/ingest.ts';
// 真实采集（活动窗口等）已【整体迁出 Core】到采集插件 plugins/collector-active-window/：
...
// [experimental] 可注入时钟：Clock 类型 + systemClock 缺省。宿主注入以求确定性 / 时间旅行；只产时间戳、不进置信度自算。
export { systemClock, type Clock } from './clock.ts';
// [experimental] config 取用方式：单例访问方式在 1.0 前可能变化；配置项形状本身 stable（见上）。
export { config } from './config.ts';

// 便携记忆包（导入/导出/备份/恢复）：让用户记忆成为可迁移资产
...
export { buildMemoryGraph, type BuildGraphDeps, type BuildGraphOptions } from './graph/index.ts';
