 ...
 *     `buildMessages` 不再把 36 字符 UUID 塞进 prompt，改发 `[e1]`（代码维护 标号↔真 id 映射、
 *     落库前翻回真 id），示例里的 `["ev-1"]` 相应改成 `["e1"]`。根因同 consolidate（D-0036）：
 *     模型模仿示例的 id 形态、间歇性把 UUID 截成前缀写回 → `candidateIds` 精确匹配落空 → 假设被
 *     静默丢弃。发标号 = 示例与真实形态一致，模型结构上写不错。**认知纪律措辞一字未动（铁律 3）**：
 *     「只给可能的原因、绝不下定论」「原因须是行为/客观观察」「至多 1 条」等全部原样，只改 id 书写形态。
 *     注：attribute 无专门的全量 eval harness（bench 里只有 consolidate 的），本版只需哈希快照 + 单测。
 *
 * 改动纪律（§15.3 / D-0009）：改内容必须 bump version；有 eval harness 的须重跑并附前后分数。
 *   认知纪律措辞（「绝不下定论」——假设只低声说）是纯位置迁移、一字不改（铁律 3）。
