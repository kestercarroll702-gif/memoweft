 ...
 *   适配器里不自造人格/人设 prompt。
 *
 * 隐私硬约束（D-0024·不可违反）：`get()` 返回的 MemoryMessage.content【只】放 buildKnowledgeBlock 的产物
 *   （只用 content/confidence/credStatus）。provenance（证据原文 + 授权位）、contentType、id、score 一律【不】进
 *   block 输出文本——provenance 进 prompt = 绕过 tier 把云受限原文喂给模型；这些字段只经 `onRecall` 交宿主。
 *
 * 降级 §16.2：`get()` 内 `withTimeout` 包 recall；超时/抛错 → 返回 `[]`（不注入）+ logger 记事件，
...
export interface MemoWeftMemoryBlockOptions {
   ...
   * 隐私硬约束（D-0024·不可违反）：provenance【绝不】进 block 输出文本——只经 onRecall 交宿主自筛。
   */
  explain?: boolean;
  ...
  /**
   * recall 超时阈值（毫秒，契约 §16.2）。缺省 200ms。超时即视为召回失败 → 降级为不注入（返回 `[]`）。
...
/**
 * MemoWeft 召回记忆块：塞进 `createMemory({ memoryBlocks:[block] })` → `agent({ llm, tools, memory })`。
...
export class MemoWeftMemoryBlock extends BaseMemoryBlock {
   ...
   * 降级 §16.2 全程包住，绝不向 Memory 抛。
   *
   * 隐私硬约束（D-0024）：返回消息的 content 只放 buildKnowledgeBlock（只用 content/confidence/credStatus）；
  ...
  async get(messages?: MemoryMessage[]): Promise<MemoryMessage[]> {
    ...
    // 观测回调 + 拼块纳入降级 guard：宿主 onRecall 或拼块万一抛错，绝不向 Memory 抛 / 掀翻这轮对话。
    try {
      onRecall?.(recalled);
      // 隐私硬约束（D-0024）：buildKnowledgeBlock 只用 content/confidence/credStatus——provenance 等绝不进 block 输出。
   ...
   *
   * 为什么这里不摄入（纪律·硬约束）：Memory 会把【整段会话消息】(含助手回话 / 已被召回注入过的上下文) 喂给
...
/**
 * 造一个 MemoWeft 召回记忆块（薄工厂，等价 `new MemoWeftMemoryBlock(core, opts)`）。
 * 塞进 `createMemory({ memoryBlocks:[block] })`，再 `agent({ llm, tools, memory })`。
 *
 * @param core 只需持有 `recall` 方法的 Core（或其最小实现）。
