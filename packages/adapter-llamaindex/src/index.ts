 ...
 *
 * 三条路径 / 两种载体：
 *   ① 召回注入（读）= `MemoWeftMemoryBlock extends BaseMemoryBlock`：塞进 `createMemory({ memoryBlocks:[block] })`
 *      → `agent({ llm, tools, memory })`。每次模型调用前 Memory 调 `block.get(messages)`，本块做语义召回、把中性
 *      知识块作为一条 `role:'memory'` 消息注入。隐私（D-0024）：provenance 绝不进 block 输出——只经 `onRecall` 交宿主。
 ...
 *      `agentToolCallEvent`（调用意图）只 re-yield 不摄（铁律 3a·by-construction）。
 *
 * 用法（工厂）：
...
export interface CreateMemoWeftLlamaIndexOptions {
  ...
  subjectId?: string;
  /** memoryBlock 注入块的语言（措辞照 Core action.ts 的 knowledgeBlock 双语口径）。缺省 'en'。 */
  lang?: 'en' | 'zh';
   ...
   *  隐私硬约束（D-0024）：provenance【只】经此回调交宿主，绝不进 memoryBlock 输出文本。 */
  onRecall?: (items: RecalledLike[]) => void;
  /** recall 超时阈值（毫秒，§16.2）。缺省 200ms。超时/抛错 → 召回降级为不注入。 */
...
export interface MemoWeftLlamaIndex {
  /** ① 召回注入（读）：塞进 `createMemory({ memoryBlocks:[memoryBlock] })`。 */
