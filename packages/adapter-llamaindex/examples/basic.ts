 ...
 *   ① 召回注入（读）= `memoryBlock`（`MemoWeftMemoryBlock extends BaseMemoryBlock`）塞进 `createMemory({ memoryBlocks:[block] })`
 ...
 *      （`agentToolCallEvent` 调用意图只 re-yield 不摄——铁律 3a·by-construction）。
 *
 * 跑法：
 *   npm i @llamaindex/core @llamaindex/workflow memoweft @memoweft/adapter-llamaindex
 *   # 真实 agent 还需一个 ToolCallLLM（如 @llamaindex/openai 的 OpenAI）+ 鉴权（OPENAI_API_KEY）+ 至少一个 tool
 *   node examples/basic.ts   # Node 22（--experimental-strip-types）；Node 23+ 原生剥类型直接 node examples/basic.ts
 * 只想看接线、不连模型：本示例的记忆三件（memoryBlock.get / persistFromAgentStream / persistUserTurn）都能离线单独调；
 *   真正的 `agent(...).runStream(...)` 段用注释示意（宿主自带 llm + tools），示例只渲染出"会被注入的记忆"并喂一条
 *   构造事件流演示写路径接线。
 */
...
// 2) 造 MemoWeft 读写四件套。lang 只影响注入块的说明文字（照 Core 中性措辞），不改 Core 行为。
...
// 3) 召回注入（读）接线：把 memoryBlock 塞进 createMemory → agent。
...
async function chatTurn(userText: string, turnId: string) {
  ...
  //        // …照常消费 ev（事件被原样透传，摄入成败完全不影响这里）…
  //      }
  //
  //    本示例不带 agent，喂一条【构造的工具返回结果事件】演示写路径接线（真实里这些事件来自 runStream）：
  const fakeStream = demoRunStream(userText);
  for await (const ev of mw.persistFromAgentStream(fakeStream, { userMessage: userText, originId: turnId })) {
    // 透传：消费者原样拿到每个事件（这里只数一下）。
    void ev;
  }
}
...
/*
 * 为何读走 memory-block、写走 stream-tap（而非硬塞别的缝）：
 *   - 读：LlamaIndex 的 Memory 机制会在每次模型调用前调各 memory-block 的 get() 取「记忆上下文」拼进 prompt——
 *     这正是召回注入的缝。本块只在 get() 里做召回、把中性知识块作为 role:'memory' 消息返回；隐私（D-0024）：
 *     provenance（证据原文 + 授权位）绝不进 block 输出，只经 onRecall 交宿主。
 *   - 写：BaseMemoryBlock 的 put() 会被 Memory 喂【整段会话消息】(含助手回话 / 已注入的记忆)——在此落库会把助手输出、
 *     甚至注入的记忆当"证据"存回去（脏数据）。故本块 put() 是空实现；写全走 persistFromAgentStream：用户原话由宿主
 *     在注入前显式传入、工具结果只认 agentToolCallResultEvent（铁律 3a）。
 */
