 ...
 *
 * 断言两件事（安全硬约束）：
 *   ① 白名单 6 个 tool 都注册了、handler 调用返回结构合理；
...
/** stub LLM：不触网，回固定话（同 tests/core.test.ts 手法）。 */
...
test('memoweft_recall / list_* / graph：读 tool 调用返回结构合理（空库不崩）', async () => {
  ...
  try {
    ...
    await client.callTool({ name: 'memoweft_ingest_user_message', arguments: { content: '我在学吉他' } });

    const recall = await client.callTool({ name: 'memoweft_recall', arguments: { query: '爱好' } });
    assert.equal(recall.isError, undefined);
    assert.ok(Array.isArray((recall.structuredContent as { result: unknown[] }).result), 'recall 返回数组');
    ...
    assert.ok(Array.isArray((listCog.structuredContent as { result: unknown[] }).result), 'list_cognitions 返回数组');

    const listEvents = await client.callTool({ name: 'memoweft_list_events', arguments: {} });
    assert.ok(Array.isArray((listEvents.structuredContent as { result: unknown[] }).result), 'list_events 返回数组');

    const graph = await client.callTool({ name: 'memoweft_graph', arguments: {} });
    const g = (graph.structuredContent as { result: { nodes: unknown[]; edges: unknown[] } }).result;
