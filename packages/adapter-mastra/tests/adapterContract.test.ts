/**
 * adapter-mastra · adapter-kit 契约接入（AD-1…AD-9）。
 *
 * 一份 kit（../../../tests/adapter-kit）喂多个适配器；这里是 Mastra Processor 侧的薄驱动：
 *   - 测试【直接调 processor.processInput / processOutputResult】、喂构造的 ProcessInputArgs /
 *     ProcessOutputResultArgs 对象——【不启动真实 Mastra Agent】、不触网、不打模型。
 *   - AD 断言由 runAdapterContract 产出（本文件是 *.test.ts，node --test 直接跑）。
 *
 * 与 A（adapter-ai-sdk）/ openai-agents 的对照：都不启动宿主运行时、都靠「直接调处理函数 + 喂构造事件」
 *   在离线核上断言（同一范式）。本包是 processor 型（processInput 召回注入 system 通道 +
 *   processOutputResult 落库用户原话/工具结果/AI 回复）。
 *
 * 铁律 3a（代码级 by-construction）：
...
// ── 离线 core（同 A / openai-agents adapterContract.test.ts 手法：stub LLM + 空召回器，:memory: 库）──
...
// onRecall 回调收到的召回项类型（透传的 v2 面：id/contentType/score/provenance）。AD-7/8 据此把
//   「透传进 onRecall 的召回对象」当断言源（非驱动自造结果）。
type OnRecallItems = Parameters<NonNullable<MemoWeftProcessorOptions['onRecall']>>[0];

// ── Mastra 事件构造夹具（喂 processInput/processOutputResult；只填适配器实际读的字段，其余 cast 略）──
...
function outputArgs(result: unknown, state: Record<string, unknown> = {}, messages: unknown[] = []): ProcessOutputResultArgs {
  return {
    messages,
    systemMessages: [],
    state,
...
function toolResultChunk(result: string, callIntent: string, callId = 'call-1'): unknown {
  // payload.args 刻意带上调用意图串——证明适配器【不读】它（铁律 3a）。
  return { type: 'tool-result', payload: { toolCallId: callId, toolName: 'get_weather', result, args: callIntent } };
}
...
// ── AD-4/7/8 离线 fake recall core：照 Core 门面 recall 语义──
...
const driver: AdapterDriver = {
  ...
  // AD-2：一轮用户原话经 processInput 捕获（注入前原文）→ processOutputResult 落库 → +1 spoken。
  ...
  // AD-1：助手侧内容流经适配器 → 零落库（by-construction）。
  //   把 processOutputResult 能拿到的助手侧全谱喂进去——result.text（助手回话）+ 一步里的 toolCalls（模型调用意图/入参）——
  ...
  async ingestAssistantTurn(text: string): Promise<number> {
    ...
    try {
      ...
      const p = createMemoWeftProcessor(core);
      await p.processOutputResult!(outputArgs({
        text,
        steps: [step({ toolCalls: [{ payload: { toolCallId: 'c1', toolName: 'get_weather', args: '{"tool":"get_weather"}' } }], toolResults: [] })],
      }, {}));
  ...
  // AD-2 幂等：同一轮稳定消息 id（= originId）经捕获→落库路径触发多次 → ingestUserMessage put 幂等去重 → 仍一条。
  ...
  // AD-3：processOutputResult 喂一步含 [toolCall(调用意图/入参), toolResult(返回结果)] →
  //   只落工具返回结果为 tool 证据（+1）；调用意图/入参（含 'get_weather'）根本不被读（铁律 3a，代码级 by-construction）。
  async ingestToolResult(resultPayload: string, callIntent: string): Promise<ToolResultTurnResult> {
    ...
    try {
      ...
      await p.processOutputResult!(outputArgs({
        text: '',
        steps: [step({
          toolCalls: [{ payload: { toolCallId: 'call-1', toolName: 'get_weather', args: callIntent } }],
  ...
  // AD-4：processInput 喂末条 user 的 ModelInput → 召回 → 注入的 system 消息文本即知识块（照 A 出 en/zh golden）。
  //   隐私（D-0024）：buildKnowledgeBlock 只用 content/confidence/credStatus，注入文本绝不含 id/contentType/score/provenance。
  ...
  //   隐私（D-0024）：provenance 只走 onRecall，buildKnowledgeBlock 不用它 → 绝不进注入 system 文本（此处断言实证）。
  async recallSurfaceExplained(fixture: RecallFixtureItem[], lang: 'en' | 'zh' = 'en'): Promise<RecallSurface> {
    ...
    for (const s of provSummaries) {
      assert.ok(!rendered.includes(s), `injected system message must NOT contain provenance summary (D-0024 privacy hard constraint): "${s}"`);
    }
    return {
      kind: 'text-block',
      rendered,
  ...
  // AD-6：故障 core → processInput 里 recall 抛错/超时（recallTimeoutMs 有界）→ 降级为【原样返回 messages】（不注入）、经 logger 记一条。
  ...
  applicability: {
    ad3: {
      status: 'applicable',
      reason: 'processOutputResult 只读 result.steps[].toolResults[].payload.result/toolCallId（工具真实返回结果）落 +1 tool 证据；payload.args（LLM 调用入参）与 result.text（助手回话）绝不读成证据（铁律 3a，代码级 by-construction，AD-3/D-0013）',
    },
    ad5: {
      status: 'na',
      reason: 'AD-5 na(mastra)：召回注入走 processInput 返回的 systemMessages（system 通道），写只读 result.steps 的 toolResults，无 LLM 输出→evidenceId 回捞落库路径（同 A/openai-agents）',
    },
    ad6: {
      status: 'applicable',
      reason: 'processInput 里 recall 抛错/超时（recallTimeoutMs 有界）降级为原样返回不注入、processOutputResult 写路径失败重试一次仍失败静默吞——都不向 Mastra 抛，经注入 logger 记一条结构化事件（契约 §16.2；throw/timeout 两模式都真跑）',
    },
    ad7: {
      status: 'applicable',
      reason: 'createMemoWeftProcessor 把 opts.contentTypes 透传进 core.recall({contentTypes}) → Core 后过滤 → onRecall 只收到匹配类型项（D-0022/D-0024，端到端透传）',
    },
    ad8: {
      status: 'applicable',
      reason: 'opts.explain 透传进 core.recall({explain}) → Core 附 provenance（含 allowCloudRead/allowInference 授权位）→ 经 onRecall 交宿主；provenance 绝不进注入 system 文本（D-0021/D-0024 隐私加固）',
    },
    ad9: {
      status: 'na',
      reason: 'AD-9 na(mastra)：processor 型读写适配器只经 processInput 召回注入 + processOutputResult 写用户原话/工具结果/AI 回复，不暴露 mute 写口；mute 负反馈是受控记忆管理（memory.mute），经宿主/B 适配器直调 Core，本适配器无此写路径（D-0023，仅 B applicable，同 A/openai-agents）',
    },
  },
};
