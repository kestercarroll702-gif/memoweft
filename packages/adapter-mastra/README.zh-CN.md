...
- **读**（`processInput`，模型调用【前】）：为用户这轮召回相关记忆，注入进 **system 通道**——绝不碰 user 消息，所以注入的内容永远不会被当成「用户说的话」回捞进来。
- **写**（`processOutputResult`，模型答【完】后）：
  - 用户这轮原话 → 一条 `spoken` 证据（在注入前捕获）；
  - 每个工具**返回结果** → 一条 `tool` 证据（只取 `payload.result`，绝不取调用入参，铁律 3a）；
...
`memoweft` 与 `@mastra/core` 是 peer 依赖。本适配器兼容 **memoweft `^0.5` 或 `^0.6`**：AI 回复 / 前文上下文这条线用的 `recordAssistantReply` 是 0.6 的面，运行时能力探测——0.5 上自动跳过，其余（召回 + 用户/工具摄入）照常工作。
...
| `recallTimeoutMs` | `200` | 召回超时（§16.2）。超时/出错这轮降级为**不注入**；读路径不重试。 |
| `logger` | — | 结构化降级事件 `{ event, op, reason }`。绝不收用户内容 / 原话 / 密钥。 |

## 保证

- **绝不注入进 user 消息。** 召回落 system 通道，捕获到的用户原话永远是干净输入。
- **铁律 3a。** 只有工具**返回结果**成为证据——工具调用入参与 AI 自己的回复都不会；AI 回复只作上下文。
- **隐私（D-0024）。** `provenance`（证据原文 + 上云/推断授权位）、`contentType`、`id`、`score` 绝不进注入 prompt——只经 `onRecall` 流转。
- **绝不阻塞对话（§16.2）。** 召回有超时上界、失败降级为不注入；写路径失败重试一次即放弃。记忆层故障永不中断一次生成。
