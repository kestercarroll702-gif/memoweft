 ...
 *       ② 工具返回结果 → tool 证据（只取 payload.result、绝不取 args/调用意图，铁律 3a）；
 ...
 *     → messages 始终干净，捕获到的原话永不含召回注入内容（等价 adapter-ai-sdk「别从注入后 prompt 回捞」纪律）。
 ...
 *     捕获→落库经 processor `state`（同一 request 内跨方法持久）传递；state 万一未跨阶段带过来，
 *     processOutputResult 从自己的 messages 兜底取（注入不碰 messages，兜底同样拿到干净原话）。
 *
 * 边界（照 MemoWeft「Core 无头」纪律）：注入文案只搬 Core 中性 knowledgeBlock 措辞；不自造人设。
 * 降级（§16.2）：recall 超时（默认 200ms）/抛错 → 不注入、经注入 logger 记一条；
...
export interface MemoWeftProcessorOptions {
   ...
   *  隐私硬约束：provenance【绝不】进注入 prompt——只经 onRecall 交宿主自筛。 */
  explain?: boolean;
  ...
  /** recall 超时阈值（毫秒，§16.2）。缺省 200ms。超时即视为召回失败 → 降级为不注入。读路径不重试。 */
  recallTimeoutMs?: number;
  /** 注入式 logger（可选，§16.2）：召回超时/抛错、写路径重试后仍失败 → 记一条结构化事件。缺省无 = 静默。
...
/** 工具返回结果 → 文本（string 原样，其它 JSON 序列化）。绝不碰 payload.args（LLM 调用入参，铁律 3a）。 */
...
/** 写路径重试一次（§16.2）：首发失败重试一次，仍失败则经 logger 记一条 ingest 降级、静默吞（不崩对话）。 */
...
/**
 * 造一个 MemoWeft 读写一体的 Mastra Processor。
 ...
 */
export function createMemoWeftProcessor(core: ProcessorCore, opts: MemoWeftProcessorOptions = {}): Processor {
  const {
  ...
  return {
    ...
    async processInput(args: ProcessInputArgs) {
      ...
      try {
        // §16.2：Promise.race 包 recallTimeoutMs（默认 200ms）超时；读路径不重试。
        // 召回 v2 透传（D-0024）：contentTypes / explain 原样交给 Core（过滤/解释都在 Core 侧做）。
    ...
    async processOutputResult(args: ProcessOutputResultArgs) {
      // 捕获轮优先取自 processInput 递来的 state；未带过来则从本阶段 messages 兜底（注入不碰 messages，原话仍干净）。
      let captured = args.state[STATE_KEY] as CapturedTurn | undefined;
      if (!captured) {
        const u = lastUserMessage(args.messages);
        if (u.text) captured = { userText: u.text, originId: u.id ?? null, conversationId: canRecordReply ? u.threadId : undefined };
      }
      ...
      // ③ 工具返回结果 → tool 证据（只取 payload.result / toolCallId，绝不取 payload.args/toolName，铁律 3a）。
      const steps = args.result?.steps ?? [];
      for (const step of steps) {
        const toolResults = (step as { toolResults?: unknown }).toolResults;
        if (!Array.isArray(toolResults)) continue;
        for (const tr of toolResults) {
          const payload = (tr as { payload?: unknown }).payload as { toolCallId?: unknown; result?: unknown } | undefined;
          if (!payload) continue;
          const text = toolResultText(payload.result);
          if (text === null) continue;
          const originId = typeof payload.toolCallId === 'string' ? payload.toolCallId : null;
          await ingestWithRetry(() => core.ingestToolResult({ content: text, originId, subjectId }), logger);
        }
      }
