/**
 * adapter-claude-agent-sdk · adapter-kit 契约接入（AD-1…AD-9）。
 *
 * 一份 kit（../../../tests/adapter-kit）喂多个适配器；这里是 Claude Agent SDK 侧的薄驱动：
 *   - 测试【直接调 hook 处理函数】、喂构造的 UserPromptSubmit / PostToolUse input 对象——【不启动真实 SDK】、不触网、不打模型。
 *   - 读写三路径都从工厂造出的 hooks 里取出对应 handler 调用：
 *       · UserPromptSubmit handler：先存用户原话（spoken）、再召回 → additionalContext（读写同一 hook）；
 *       · PostToolUse handler：存工具结果（tool），只读 tool_response / tool_use_id（铁律 3a，绝不碰 tool_input）。
 *   - AD 断言由 runAdapterContract 产出（本文件是 *.test.ts，node --test 直接跑）。
 *
 * 与 A（adapter-ai-sdk）的对照：A 是进程内 middleware/onEnd 型，本包是进程内 hooks 型；
 *   两者都不启动宿主运行时、都靠「直接调处理函数 + 喂构造事件」在离线核上断言（同一范式）。
 */
...
// ── 离线 core（同 A / mcp server.test.ts 手法：stub LLM + 空召回器，:memory: 库）──
...
// createMemoWeftAgentHooks 期望的 core 面（AgentCore 未导出，从工厂签名取），fake core 据此 cast。
...
const HOOK_BASE = { session_id: 'sess-1', transcript_path: '/tmp/transcript.jsonl', cwd: '/tmp' } as const;
...
/** 构造一个 PostToolUse input：tool_response=工具真实返回（应落库）；tool_input=LLM 调用意图/入参（绝不落库，铁律 3a）。 */
function ptuInput(toolResponse: unknown, toolInput: unknown, toolUseId = 'call-1'): PostToolUseHookInput {
  return {
    ...HOOK_BASE,
...
function handlersOf(core: AgentCoreArg, opts?: MemoWeftAgentHooksOptions): { ups: HookCallback; ptu: HookCallback } {
...
// ── AD-4/7/8 离线 fake recall core：照 Core 门面 recall 语义（createCore.ts:359-380）──
...
function fakeRecallCore(fixture: RecallFixtureItem[]) {
  return {
    async ingestUserMessage() { return {}; },
...
const driver: AdapterDriver = {
  ...
  // AD-2：一轮用户原话经 UserPromptSubmit handler 落库 → +1 spoken。
  //   注入走返回值 additionalContext、绝不改 input.prompt → 存进证据的原话永不含召回注入内容（by-construction 干净）。
  ...
  //   故无 handler 可喂助手文本：跑一遍真核的读写路径（PostToolUse 只吃 tool_response）也绝不会把助手内容当证据。
  async ingestAssistantTurn(_text: string): Promise<number> {
    const core = makeCore();
    try {
      const before = core.memory.listEvidence({}).length;
      // 守结构不变量（非空断言，别让 AD-1 沦为形同虚设）：适配器【只】注册 UserPromptSubmit（用户原话）
      //   + PostToolUse（工具结果）两个 hook，绝无任何会携带助手输出的 hook（如 Stop 的 last_assistant_message、
      //   或读 SDKAssistantMessage 的路径）——助手输出永不成为证据（铁律 3a）。若将来误加了这类 hook,此断言即红。
      const { hooks } = createMemoWeftAgentHooks(core);
      assert.deepStrictEqual(
        Object.keys(hooks).sort(),
        ['PostToolUse', 'UserPromptSubmit'],
        'adapter must mount only UserPromptSubmit + PostToolUse — no hook may carry assistant output (iron rule 3a)',
      );
      // 无助手摄入路径 → 无落库。恒 0（by-construction）。
  ...
  // AD-2 幂等：同一轮稳定 prompt_id（→ originId），UserPromptSubmit handler 触发多次 → put 幂等去重 → 仍一条。
  async ingestUserTurnIdempotent(text: string, times: number): Promise<number> {
    const core = makeCore();
    try {
      const before = core.memory.listEvidence({}).length;
      const { ups } = handlersOf(core);
      for (let i = 0; i < times; i++) await ups(upsInput(text, 'stable-turn'), undefined, hookCtx());
      return core.memory.listEvidence({}).length - before;
    } finally {
      core.close();
    }
  ...
  // AD-3：PostToolUse handler 喂 { tool_response=结果, tool_input=调用意图 } →
  //   只落工具返回结果为 tool 证据（+1）；tool_input（'get_weather' 意图/入参）根本不被读（铁律 3a，代码级 by-construction）。
  ...
  // AD-4：UserPromptSubmit handler 召回 → 返回值 additionalContext 即注入面文本块（照 A 出 en/zh golden）。
  //   隐私（D-0024）：buildKnowledgeBlock 只用 content/confidence/credStatus，additionalContext 绝不含 id/contentType/score/provenance。
  async recallSurface(fixture: RecallFixtureItem[], lang: 'en' | 'zh' = 'en'): Promise<RecallSurface> {
    const core = {
      async ingestUserMessage() { return {}; },
      async recall(): Promise<RecalledCognition[]> { return fixture as unknown as RecalledCognition[]; },
  ...
  // AD-7：带 contentTypes 调召回 → fakeRecallCore 后过滤 → handler 把选项透传进 core.recall → onRecall 只收到匹配类型项。
  ...
  // AD-8：带 explain 调召回 → fakeRecallCore 逐条附 provenance → handler 透传 explain → 经 onRecall 交宿主。
  //   surface.items 取自 onRecall 捕获对象，携带 provenance（含 allowCloudRead/allowInference 授权位）供断言。
  //   隐私（D-0024）：provenance 只走 onRecall，buildKnowledgeBlock 不用它 → 绝不进注入 additionalContext。
  ...
  // AD-6：故障 core → UserPromptSubmit handler 降级不抛、经注入 logger 记一条。
  ...
  async runWithFaultyCore(mode: FaultMode): Promise<FaultOutcome> {
    ...
    // 隔离 recall 降级（别让 ingest 降级掩盖 recall 日志回归）：除 logged>0，断言确有一条 recall 面降级事件。
    assert.ok(
  ...
  applicability: {
    ad3: {
      status: 'applicable',
      reason: 'PostToolUse hook 只读 tool_response/tool_use_id → ingestToolResult 落 +1 tool 证据；tool_input（LLM 调用意图/入参）绝不解构/引用（铁律 3a，代码级 by-construction，AD-3/D-0013）',
    },
    ad5: {
      status: 'na',
      reason: 'AD-5 na(claude-agent-sdk)：hooks 只读构造的 input、注入走返回值 additionalContext，无 LLM 输出→evidenceId 回捞落库路径（同 A）',
    },
    ad6: {
      status: 'applicable',
      reason: 'UserPromptSubmit hook 里 recall 抛错/超时降级为不注入、ingest 失败重试一次仍失败静默吞——都不向 SDK 抛，经注入 logger 记一条结构化事件（契约 §16.2；throw/timeout 两模式都真跑）',
    },
    ad7: {
      status: 'applicable',
      reason: 'createMemoWeftAgentHooks 把 opts.contentTypes 透传进 core.recall({contentTypes}) → Core 后过滤 → onRecall 只收到匹配类型项（D-0022/D-0024，端到端透传）',
    },
    ad8: {
      status: 'applicable',
      reason: 'opts.explain 透传进 core.recall({explain}) → Core 附 provenance（含 allowCloudRead/allowInference 授权位）→ 经 onRecall 交宿主；provenance 绝不进注入 additionalContext（D-0021/D-0024 隐私加固）',
    },
    ad9: {
      status: 'na',
      reason: 'AD-9 na(claude-agent-sdk)：hooks 型读写适配器只经 UserPromptSubmit 召回注入 + PostToolUse 写工具结果，不暴露 mute 写口；mute 负反馈是受控记忆管理（memory.mute），经宿主/B 适配器直调 Core，本适配器无此写路径（D-0023，仅 B applicable，同 A）',
    },
  },
};
