...
发布形态已就绪：`package.json` 已配 `main=dist/index.js`、`types=dist/index.d.ts`、`files` 白名单（只挑 `dist` 的 `.js`/`.d.ts` + 双 README + CHANGELOG + LICENSE）、`type=module`、`engines.node>=24`、零 runtime 依赖。

步骤：

1. **定版本号**：`package.json` 的 `version` 与 `src/version.ts` 的 `MEMOWEFT_VERSION` **两处一起改**——没有自动化替你对齐，这是人工纪律。1.0 前 minor 也可能带破坏性改动，按实际影响进位。
2. **发布保险丝已就位**：scripts 里的 `prepublishOnly`（`typecheck && test && build`）在 `npm publish` / `npm pack` 打包前**自动**跑三绿 + 重新构建，任一步红则中止发布。要手动先看一眼绿不绿也行，但**不必**再手动重跑一遍——陈旧 `dist/` 由这道闸兜住。
3. **核对打包内容**：`npm pack --dry-run`（会顺带触发 `prepublishOnly`），确认只含 `dist` 的 `.js`/`.d.ts` + 双 README + CHANGELOG + LICENSE，无 `.map`/`src`/`.env`/`*.db`。
