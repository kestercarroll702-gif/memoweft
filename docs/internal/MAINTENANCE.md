...
| [`项目地图.md`](项目地图.md) | 设计 master（17 格全貌 + 决策） | **按 cell 随用随读，别通读**（省 token） |
...
⑤ 测试（三绿）   npm run typecheck && npm test && npm run build 全绿；自查对照地图规则
      ↓
⑥ docs-sync      改写 STATE.md + 历史/决策记进提交说明 +（决策变了才）改地图 cell + 同步对外 docs
      ↓
...
8. **记里程碑**：更新 `CHANGELOG.md`（对外可读的里程碑）；细节留在 git 提交历史里。
