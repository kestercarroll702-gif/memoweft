...
- [ ] Schedule `core.expire()` as a periodic maintenance job (for example daily, or immediately before a scheduled `core.updateProfile()`). It marks transient cognitions (`state`/`hypothesis`/`trend`) that have passed their `expireAfterDays` window as invalid so they stop being recalled. It is idempotent and rule-only (no LLM or embedder), does not delete (it sets `invalidAt`, kept traceable), and is intentionally decoupled from `updateProfile`—without a scheduled call, transient memory never expires. Note that `expire` does not rebuild the recall index, so prefer running it *before* a profile update (whose reindex then drops the just-invalidated items) rather than *after*: items invalidated after the last reindex stay in the index until the next `updateProfile`. They are always filtered out at recall time (never returned), but while indexed they can occupy the recall over-fetch pool and reduce how many valid memories a query can surface.
