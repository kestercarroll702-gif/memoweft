...
MemoWeft 持有的每一条判断，都能追溯回产生它的原始话语。它记录的不只是*知道什么*，还有*这来自哪里*。
...
- **Evidence（证据）** —— 原始材料。被说出、被观察、被推测，或由工具返回的内容。它带有 `sourceKind`、时间戳和授权标记。它从不持有判断。见 [`src/evidence/model.ts`](../../src/evidence/model.ts)。
- **Event（事件）** —— 对一段对话切片的带上下文摘要，链回它所覆盖的证据。判断由事件构建（事件携带上下文）；而追溯的落点仍然是原始话语。
- **Cognition（认知）** —— 关于用户的一条判断：事实、偏好、目标、状态等等。每条认知都保留一个 `sources` 列表，记录**支持**或**反驳**它的证据。

在信任上这条链只指向一个方向：一条认知的可信度，不会超过它下面的证据。抽掉证据，判断就失去了根基。
