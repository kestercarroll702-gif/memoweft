...
| 要求 | 说明 |
| --- | --- |
| **Node ≥ 24** | 库直接跑 `.ts`、用 Node 内置 `node:sqlite`（作者实测 **v24**；CI 也锁 24）。`node:sqlite` 到 24 才转正、原生跑 `.ts` 也到 24 才稳定；22/23 上这两样一般能用、但只打实验警告、不保证稳定，锁 24 是为稳定。用 `node --version` 确认。 |
...
npm test            # 69 个测试全过（作者实测 69 passing）
...
**Q：跑 `npm test` 或 `node examples/minimal.ts` 报 `node:sqlite` 找不到 / 跑不了 .ts、或刷一堆实验警告？**
A：多半是 Node 版本偏低。升级到 **≥ 24**（作者实测 v24，CI 也锁 24）。`node:sqlite` 与原生 `.ts` 执行到 24 才转正、才稳定；22/23 上一般能跑、但只打实验警告、不保证稳定，锁 24 是为稳定。
